...
**Failure 2 -- The registry re-creates instead of reusing.** For controllers like ReasoningBank, CausalRecall, LearningSystem, and ExplainableRecall, the registry constructs new instances rather than fetching AgentDB's existing ones. The comment at line 883 acknowledges this: "AgentDB.getController() only supports: reflexion/memory, skills, causalGraph/causal". Rather than extending AgentDB's getController() to cover all controllers, the registry worked around the gap by direct construction. This doubled the instance count.
...
**Part A -- AgentDB becomes a lazy container.** AgentDB.initialize() should set up infrastructure (database, embedder, vector backend) but NOT eagerly create domain controllers. Controllers should be created on first access via getController(), using the ADR-0040 constructor injection pattern to share singletons. This eliminates the "double creation" problem.

**Part B -- The registry becomes the sole controller factory.** All controller construction goes through ControllerRegistry.createController(). For AgentDB-hosted controllers (those that need the raw database handle), the registry creates them and hands them to AgentDB for storage. For CLI-layer controllers, the registry creates and manages them directly. The `get<T>()` method remains the single access point.
...
**Where the intent is incomplete or contradictory**: The ADR-0040 pattern (accept pre-created singletons) and the ADR-053 pattern (registry wraps AgentDB) are structurally at odds. ADR-0040 assumes AgentDB is the outer container that receives singletons. ADR-053 assumes the registry is the outer container that delegates to AgentDB. Both cannot be the outermost layer simultaneously. The resolution is Part A above: AgentDB becomes an inner container that exposes its database and infrastructure, while the registry owns the controller lifecycle. This was the trajectory suggested by the ADR-053 header comment but never carried through to AgentDB.ts itself.
