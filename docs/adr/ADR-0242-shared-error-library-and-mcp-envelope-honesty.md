...
Pattern-1 Council Hive (Dialectic). Weighted consensus (queen ×3). 6 experts + Devil's Advocate. Hierarchical-mesh topology. Strategic queen. Queen-composed transport. Per [[ADR-0233]] §Per-ADR swarm configuration row for CT-I.

### Verdict

**Adopt Option A + Option B as drafted**, with **4 improvements** surfaced by the panel and **2 substantive corrections** to load-bearing claims in the ADR's pre-flight verification. Weighted tally (queen vote ×3 + 5 worker votes = 8 total weighted points; threshold `ceil(8/2) = 4`):

| Voter | Weight | Vote | One-line position |
|-------|--------|------|-------------------|
| **Queen** (strategic) | 3 | **adopt-with-improvements** | The canon + lint + arch-test triple is the right shape; the long-term framing pre-empts the "rot like dead retry libs" risk; the improvements below tighten the structural claims without re-scoping. |
| E1 (error-class hierarchy) | 1 | adopt | `gastown-bridge/errors.ts` is the right gold standard; the `GasTownError`/`BeadsError`/`ValidationError`/`CLIExecutionError`/`FormulaError`/`ConvoyError` hierarchy uses every modern shape (`code`/`cause`/`context`/`timestamp`/`captureStackTrace`/`toJSON`/`toString`/type guards + `wrapError` adapter). |
| E2 (MCP-envelope) | 1 | adopt-with-amendment | The arch-test rule (handler-top-level `catch → {success:false}` w/o discriminator) is the right narrow shape; ADR must explicitly note disjointness from [[ADR-0247]] (CT-N F-04-009, client-side `callMCPTool` `isError`) — they share the protocol-boundary concern but at opposite ends. |
| E3 (lint-grandfathering) | 1 | adopt-with-amendment | Advisory-first pattern matches `check-silent-catches.mjs` / `check-undiscriminating-catches.mjs` precedent; FP-rate-zero promotion criterion is sound; allowlist must be content-keyed (matching the `fcab2bc` refactor of `undiscriminating-catches-allowlist.txt`) — the ADR says this but the script-name reference should be explicit. |
| E4 (retry-library consolidation) | 1 | adopt-as-out-of-scope | Correctly deferred per `feedback-corpus-evidence-before-feature-work` — picking between `production/retry.ts` and `shared/resilience/retry.ts` needs its own swarm review. The 8 ad-hoc retry loops also need per-site disposition. F-13-001 is its own future ADR. |
| E5 (migration-pace) | 1 | adopt | Long-term framing is honest about what doesn't migrate; the "~1,994 throws stay" disclosure prevents future maintainers from misreading the ADR as a migration commitment. Cycle-1 scope (extract package + ship advisory checks) is small enough to land in one release cycle. |
| **DA** (devil's advocate) | 1 | **hold-principled-dissent** | "Shared error library will rot just like the dead retry libs unless mandatory adoption is gated, but mandatory adoption is exactly what the ADR explicitly disclaims." AND: "Lint-grandfathering accepts perpetual bifurcation — accept the cultural debt instead of paying ongoing review-time cost to enforce a two-shape codebase." Both addressed below; DA accepts the majority verdict but records dissent for the [[ADR-0233]] follow-up tracker. |

**Result**: queen (3) + 5 adopt votes = **8 weighted points adopt** (threshold 4 cleared). DA holds principled dissent (not a vote against; recorded in tracker). Decision ratified.

### Improvements adopted (4)

1. **(E2 + queen)** — Add explicit disjointness paragraph vs [[ADR-0247]] (CT-N) to the §"vs sibling ADRs" pre-flight section. The current draft references ADR-0209 and ADR-0210 but pre-dates ADR-0247's ratification. ADR-0247's own pre-flight (per the CT-N swarm review fragment) already documents this disjointness explicitly: ADR-0242 owns the **server-side** rule "MCP handlers must throw fatals, not catch-and-return-`{success:false}`"; ADR-0247 owns the **client-side** rule "`callMCPTool` must inspect `isError: true` envelopes and throw, not return-as-T". Same protocol-boundary concern, opposite ends. **Disjoint by artifact** (handlers vs `callMCPTool`) **and by mechanism** (arch-test on swallow-shape vs runtime `isError` inspection).

2. **(E3 + queen)** — Make the allowlist script names + content-keying convention explicit in the §Confirmation source-shape gate. The ADR currently says "modelled on the existing zero-dep `scripts/check-silent-catches.mjs` / `check-undiscriminating-catches.mjs` family" and "content-keyed, matching the existing `undiscriminating-catches-allowlist.txt` shape after the `fcab2bc` refactor." The Confirmation gate should name the new files explicitly: `lib/throw-new-error-allowlist.txt` (content-keyed) and `lib/mcp-handler-fatal-throw-allowlist.txt` (handler-id-keyed; baseline = today's ~56 sites). Both registered in `scripts/ruflo-publish.sh` alongside the existing checks per `[[reference-acceptance-runcheck-vs-collect]]` (both `run_check_bg` AND `collect_parallel` to avoid the silent-no-verdict trap).

3. **(Queen)** — Add an INTEGRATION-LEDGER row commitment to §Confirmation. The new `@claude-flow/errors` package re-exports `gastown-bridge/errors.ts` content; per `[[feedback-update-integration-ledger]]` the ledger row is owed when the package lands. **Disposition**: `convergence-with-upstream` (the seed code is byte-identical with upstream's `ruvnet/ruflo/v3/plugins/gastown-bridge/src/errors.ts`; the new package is a structural reorganization that re-exposes upstream-derived code under a shared boundary — not a fork-divergent invention). The plugin-side `gastown-bridge/errors.ts` re-export-from-shared also commits to a `convergence-with-upstream` row (refactor preserves byte-identical behavior at consumer call sites).

4. **(E5 + DA-counter)** — Add a §"Erosion-vs-rot disposition" subsection to the ADR's More information section. DA's challenge ("Shared error library will rot just like the dead retry libs") is the load-bearing risk. The dead-retry-libs failure mode was: zero callers because (i) no consumer-facing forcing function (no lint, no arch-test, no review-time signal), and (ii) richer/cleaner-than-needed for the actual ad-hoc loop use cases. **This ADR's mitigation is structurally different on both axes**: (i) the advisory-first lint creates a review-time signal on every new `throw new Error(string)` site; new code authors are pointed at the canon at edit time, not buried in a "see also" link in a USERGUIDE section; (ii) the canon is **exactly** the existing `GasTownError` shape (which already has 2 plugin-family adopters: `gastown-bridge` itself and `agentic-qe` via inheritance) — not a richer-than-needed library design. The minimum-viable subset is what gets extracted; the impl swarm picks the `RUFLO_E*` naming convention (or whatever scheme survives review). **Recorded mitigation invariant**: if the lint advisory count is still > 0 in cycle N+3 (three release cycles post-adoption), the ADR's follow-up sweep should re-examine whether the canon's adoption rate is non-zero (i.e. is the canon being *imported* by new code, even where the lint hasn't fired?). If adoption is zero across all new code, that's the "rotting like retry libs" signal; otherwise the canon is working as designed and the long-tail of existing throws is exactly the documented erosion-not-replacement path.

### Substantive corrections (2)

These correct load-bearing claims in the ADR's pre-flight verification; both surface upstream alignment data the original draft underweighted.

1. **(E1, E5, queen — UPSTREAM ALIGNMENT)** — The ADR's pre-flight check #2 ("Upstream hasn't already decided it") correctly notes upstream carries the byte-identical `sanitizeError` definition and the `~480`-LOC `production/error-handler.ts` (fork has 480 LOC; **upstream has 398 LOC** — the fork has *extended* the dead facility, deepening F-13-002 without wiring any callers; this is a minor but worth-noting drift). But the ADR understates the gold-standard story: **`gastown-bridge/errors.ts` is upstream code**. The fork file is 701 lines; upstream `/Users/henrik/source/ruvnet/ruflo/v3/plugins/gastown-bridge/src/errors.ts` is 700 lines — byte-identical for the hierarchy subset this ADR proposes to extract. The plugin is **published as `@claude-flow/plugin-gastown-bridge@0.1.4`** on Verdaccio (confirmed via `curl http://localhost:4873/@claude-flow/plugin-gastown-bridge`). **Implication for the merge-tax claim in §Consequences "Neutral / Bad"**: the merge tax is even smaller than the ADR claims. The new `@claude-flow/errors` package is a re-export of code that originated upstream and is currently shipped via two upstream-aligned plugins. The introduction is structural (a new package boundary inside the fork's `forks/ruflo/v3/@claude-flow/*` layout); the *content* is upstream-derived. Per [[ADR-0234]] precedent, a divergence-marker comment in the new `@claude-flow/errors/src/index.ts` + the `gastown-bridge/errors.ts` re-export shim is sufficient. **The ADR should add a sentence to §Pre-flight verification step 2** acknowledging the upstream-derived provenance and committing to the INTEGRATION-LEDGER `convergence-with-upstream` row.

2. **(E4 — RETRY-LIBRARY UPSTREAM PARITY)** — The ADR correctly defers F-13-001 (retry-library consolidation) to a future micro-ADR. Confirming: both retry libraries exist in upstream too (`ruvnet/ruflo/v3/@claude-flow/cli/src/production/retry.ts` and `ruvnet/ruflo/v3/@claude-flow/shared/src/resilience/retry.ts`), both with zero production callers in either fork or upstream. The F-13-001 deferral is therefore **upstream-aligned by omission** — picking one library and deleting the other is a fork-only intervention that would carry permanent merge tax. The future micro-ADR will need to make that call explicitly. **No change required to ADR-0242**; this is purely confirmatory data for the deferral rationale.

### DA principled dissent (recorded)

DA's two challenges, both formally answered:

**DA challenge 1**: *"Shared error library will rot just like the dead retry libs."* Addressed by Improvement #4 (Erosion-vs-rot disposition): structural difference is (i) the advisory lint creates a review-time forcing function the retry libs never had, and (ii) the canon is minimum-viable (extracting exactly what 2 plugin families already use, not a richer-than-needed library design). Mitigation invariant: cycle N+3 adoption-rate check.

**N+3 baseline observation (2026-05-28 swarm, ~4 days post-landing — too early to call, but recorded so the recheck has an anchor):** genuine new importers of `@sparkleideas/errors` (excluding the package itself + the gastown-bridge re-export shim) = **0**. Advisory lint `check-throw-new-error.mjs`: 892 scanned / 781 grandfathered / **6 delta-from-baseline** — all 6 are NEW naked `throw new Error(string)` added by ADR-0255 (`memory-tools.ts:1370,1373,1376`) + ADR-0261 (`hooks-tools.ts:1707,2858`, `agentdb-tools.ts:2590`), i.e. new features ignored the canon (mildly negative signal). BUT the grandfather allowlist `lib/throw-new-error-allowlist.txt` has NOT been padded since landing (1 commit, never modified) — the worst failure mode (silent grandfathering) is absent; the 6 sit as honest un-squelched delta. **Verdict: too early (4 days, priority-10 long-tail ADR), not rot.** Recheck after the next 3–4 net-new MCP-tool/handler features land (= cycle N+3 of *new error-handling code*, not calendar days). Decision rule at recheck: still-0 importers + climbing delta + allowlist padding → nudge (promote the narrow fatal-rethrow shape, or point new handlers at `wrapError`); any adoption → working as designed. Cheapest pre-emptive nudge if wanted: `agentdb-tools.ts:2590` ("embedding service returned empty") is a real fatal closer to the data-integrity class than the ADR-0255 not-implemented guards — a ready single-site starter for `wrapError`.

**DA challenge 2**: *"Lint-grandfathering means perpetual bifurcation — accept the cultural debt instead."* Held principled dissent; argues the review-time cost of explaining the two-shape codebase ("new code uses `RufloError`, old code uses naked throws — for now") is non-trivial and that an honest "this is the cultural debt; live with it" disposition would be cleaner than an advisory-first detector that may never promote to `exit 1`. DA acknowledges the majority verdict (the lint cost is small; the canon's existence enables future targeted migrations like F-13-006's 41 message-interp throws) but records the position for the [[ADR-0233]] follow-up tracker. If after 3 cycles the lint hasn't promoted to `exit 1` AND the canon adoption rate is non-zero AND the advisory count is materially growing (not just plateauing), DA's dissent is vindicated and the canon may indeed need a "delete-and-accept-the-debt" follow-up ADR.

### Key upstream finding (per assignment)

**`forks/ruflo/v3/plugins/gastown-bridge/src/errors.ts` is NOT fork-only — it IS upstream code** (byte-identical between fork's 701 LOC and upstream's 700 LOC for the hierarchy this ADR extracts). The plugin ships as `@claude-flow/plugin-gastown-bridge@0.1.4` on Verdaccio and is listed in `featured/trending/official/newest` per `cli/scripts/publish-registry.ts` and `discovery.ts`. **Implication**: the prompt's hint that gastown-bridge is "fork-only per CT-O finding" is incorrect — CT-O ([[ADR-0248]]) treats gastown-bridge as a **published plugin** subject to per-plugin disposition (delete/publish/rewrite), and CT-F ([[ADR-0239]]) explicitly *defers* gastown-bridge's deletion specifically because it's a published artifact whose source is upstream. **This makes the ADR-0242 extraction *easier*, not harder**: the new `@claude-flow/errors` package re-exports code that originated upstream, so the merge-tax against future upstream syncs is structural (a new package boundary) rather than substantive (no upstream-divergent content). The `gastown-bridge/errors.ts` re-export shim preserves byte-identical behavior at the plugin's consumer call sites. INTEGRATION-LEDGER row disposition: `convergence-with-upstream` (re-organization of upstream-derived content under a shared boundary), not `superseded-by-local` (which would imply fork-divergent invention).

The secondary upstream finding: **fork's `production/error-handler.ts` has 480 LOC; upstream's is 398 LOC** — the fork has extended the dead facility by ~82 LOC without wiring any callers. This is a minor but worth-noting drift that the F-13-002 follow-up ADR's "wire-or-delete" decision should weigh. (If "delete" wins, the deletion closes 480 fork-LOC; if "wire" wins, the wire must happen on the upstream-aligned 398-LOC version to minimize merge tax.) No change required to ADR-0242 itself; this is data for the F-13-002 follow-up.

### Cross-references

- [[ADR-0233]] §CT-I — defect-class origin; priority 10 (lowest urgency); long-term framing consistent with that calibration.
- [[ADR-0201]] §"Remediation-ADR pre-flight checklist" — ran with the 2 corrections above (gastown-bridge upstream-derived provenance; retry-libs upstream parity confirmatory).
- [[ADR-0209]] — owns per-site bulk-fix of dishonest-success envelopes; this ADR owns the protocol-boundary mechanism for fatals. Disjoint per ADR-0242 §pre-flight check #4.
- [[ADR-0210]] — second-pass council explicitly identified this micro-ADR as owed for the protocol-boundary signal; this ADR delivers the envelope-honesty half.
- [[ADR-0247]] (CT-N) — client-side complement (`callMCPTool` `isError` propagation). **Disjoint by artifact** (handlers vs `callMCPTool`) **and mechanism** (arch-test on swallow vs runtime `isError` inspection). Both ADRs target propagation of fatals through the MCP envelope; ADR-0242 owns the server-side handler-rule, ADR-0247 owns the client-side helper-rule. Folded into Improvement #1 above.
- [[ADR-0234]] — divergence-marker comment precedent (used here for the new `@claude-flow/errors/src/index.ts` provenance comment).
- [[ADR-0239]] (CT-F) — defers gastown-bridge deletion explicitly because it's a published artifact whose source is upstream; this ADR's extraction is therefore *also* not a delete.
- [[ADR-0248]] (CT-O) — owns gastown-bridge per-plugin disposition; this ADR's extraction does not conflict (the plugin's runtime behavior is preserved via the re-export shim).
- `[[feedback-update-integration-ledger]]` — INTEGRATION-LEDGER rows owed for both fork edits (new package + gastown-bridge re-export shim); both rows are `convergence-with-upstream`.
...
- `[[feedback-skip-accepted-as-squelch]]` — the advisory-first lint is NOT a squelch (modelled on ADR-0209's settled approach); if it never promotes to `exit 1`, that itself is data per Improvement #4's mitigation invariant.
- `[[feedback-corpus-evidence-before-feature-work]]` — why F-13-001 (retry consolidation), F-13-002 (ErrorHandler wire-or-delete), F-13-003 (`ErrorCodes` dedupe), F-13-004 (naming convention) are deferred to follow-up swarm reviews, not picked unilaterally here.
- `[[feedback-commit-forks-before-release]]` — both fork edits (new package + re-export shim) commit BEFORE the next `npm run release`.
- `[[reference-acceptance-runcheck-vs-collect]]` — both new check scripts (`check-throw-new-error.mjs`, `check-mcp-handler-fatal-throw.mjs`) register in BOTH `run_check_bg` AND `collect_parallel` lists per Improvement #2.
