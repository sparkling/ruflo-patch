...
**Release gate (status: implemented + released (patch.383) + installed-artifact-verified — CLOSED).** Consumer commit `01d1feac7` shipped in **`@sparkleideas/memory@3.0.0-alpha.15-patch.383`**: a fresh `/tmp` install of the published package carries `_maybeReloadFromDisk` in `dist/rvf-backend.js` (8 occurrences), and its transitive optional dep **`@sparkleideas/ruvector-rvf-node@0.1.7-patch.222`** supplies `peekTxnid`/`openReadonly` (binding resolves from the install's `node_modules` via the dist's bare `import('@sparkleideas/ruvector-rvf-node')` — no symlink). The smoke `scripts/smoke-adr0274-xprocess-freshness.mjs` now exercises the **`ACCEPT_TEMP` installed artifact**, matching its sibling `adr0274-rvf-rw-split`: it resolves `RvfBackend` from `node_modules/@sparkleideas/memory/dist/rvf-backend.js` under the shared install (`ADR0255_SMOKE_SHARED_TEMP`), or self-installs `@sparkleideas/memory@latest` from the registry when run standalone. It is discriminating: **5/5 PASS against patch.383** (incl. the `count()` enumeration assertion), **3 FAIL against the pre-fix patch.382 dist** (zero `_maybeReloadFromDisk` → stale view). The smoke is wired into the canonical harness (`run_check_bg` + `collect_parallel` in `scripts/test-acceptance.sh`; check fn `check_adr0274_xprocess_freshness` in `lib/acceptance-adr0274-checks.sh`).
