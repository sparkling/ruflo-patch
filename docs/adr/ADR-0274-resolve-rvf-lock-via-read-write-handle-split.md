...
### Amendment: cross-process content-freshness wired (D3/D4) — 2026-05-31

The "implemented + deployed (2026-05-30)" amendment's last bullet recorded cross-process *content* freshness as an unsolved consistency window: *"bounded by the in-memory `entries` Map … Recorded as the consistency window (D6), not solved here."* That gap is now **closed on the read path**. The single-handle park/unpark deviation (D5) had kept queries serving lock-free from the parked handle's *boot-time* `entries`, so a peer process's later commit stayed invisible — the literal "183 vs 281" / "183 adr-patterns vs 281" symptom from caveat #2 of the session handover. The D3/D4 wiring that the deviation dropped is now in place. Fork commit `01d1feac7` (`@claude-flow/memory/src/rvf-backend.ts`). Fork builds clean; fork-verified FAIL→PASS via a new smoke (below).

- **Completes D3/D4 wiring the D5 deviation dropped.** The O(1) `peekTxnid` signal already existed (shipped by WS1 / ruvector `8fb99c02b`) but was wired to nothing on the read path. A new `_maybeReloadFromDisk()` is now called at the **top of `query()`, `search()`, `count()`, AND `listNamespaces()`**. The latter two cover the `memory_stats` → `case 'stats'` → `count(ns)` path (per-namespace count via `listNamespaces()`) — the literal "183 vs 281" enumeration symptom; `query()`/`search()` cover the list and semantic-search paths (D4's lazy-on-read choke point, now four methods instead of the originally-scoped two).
- **Mechanism (lock-free, no native re-ingest).** `_maybeReloadFromDisk()` compares the O(1) static `RvfDatabase.peekTxnid(path)` against a cached `_lastSeenTxnid`; on advance it opens a lock-free static `RvfDatabase.openReadonly(path)` and re-reads committed state via `_collectNativeSnapshots`/`_populateFromSnapshots` (both refactored out of `init` for reuse), rebuilding the JS `entries`/`keyIndex` (+`hnswIndex`). It **never touches `this.nativeDb` and never re-ingests native vectors**, so there is no orphan `.rvf` growth.
- **Guards (correctness + cost).** Self-write skip (`_lastOwnWriteTxnid`, set by `_noteOwnWrite()` after local commits — a reader never reloads to observe its own just-written txnid); lock-depth skip (no-op while `_lockHeldDepth > 0`); 250 ms debounce (`_RELOAD_DEBOUNCE_MS`); feature-detect (whole path no-ops if the binding lacks the `peekTxnid`/`openReadonly` statics). A one-shot `_freshnessReloadAddedEntries` flag forces a brute-force search supplement on the next `search()` so peer entries surface in semantic candidacy.
- **Scope.** 2a (list/count/stats *enumeration* freshness) is FULLY fixed. 2b (semantic *candidacy* for peer entries) is handled at current scale via the existing orphan self-heal supplement (the one-shot flag). The Rust delta-segment reader stays **DEFERRED** as the large-corpus optimization: a full reload is O(vectors), but it is bounded here by the debounce + lazy-on-read + self-write guards, and — critically — **reads do not advance the txnid**, so this does NOT reintroduce the write-side O(N²) park/unpark cascade that the D5 deviation's `resync_for_write` was engineered to avoid.
- **This supersedes the earlier "not solved here" framing for reads.** The D6 consistency window's *read* side is now closed via the lazy peek-reopen: a long-lived reader (e.g. the MCP server) reflects a peer process's writes within one debounce window without a restart. The window narrows to the debounce interval; the write-side coordination (park/unpark + flock cycle) is unchanged.

**Honest release-gate caveat (status: implemented + fork-verified; release verification pending — NOT deployed/released).** The new smoke `scripts/smoke-adr0274-xprocess-freshness.mjs` (5/5 PASS post-fix, incl. the `count()` enumeration assertion; FAIL pre-fix) is **FORK-DIST-pinned**: it loads `forks/ruflo/.../@claude-flow/memory/dist` plus the built `@ruvector/rvf-node` crate. This is UNLIKE its sibling `adr0274-rvf-rw-split`, which exercises the `ACCEPT_TEMP` *installed* artifact. So this smoke verifies the fork SOURCE, not the shipped package. Installed-artifact verification is the remaining **RELEASE GATE**. The binding question is now RESOLVED (2026-05-31 investigation): the release rebuilds the napi crate locally (`scripts/napi-rebuild.sh` + `lib/napi-config.sh:29`) and the codemod renames `@ruvector/` → `@sparkleideas/ruvector-` (`scripts/codemod.mjs:30-31`), so production resolves **`@sparkleideas/ruvector-rvf-node@0.1.7-patch.222`** (NOT the stale upstream `@ruvector/rvf-node@0.1.8` on Verdaccio, which production never touches). A fresh `/tmp` install of `@sparkleideas/memory@patch.382` was verified — `strings` + loader-resolution + live execution — to carry `peekTxnid`, `openReadonly`, `releaseLock`, `reacquireLock`, `park_writer`. So the binding substrate is shipping-ready. The ONLY remaining gap: this consumer commit (`01d1feac7`) post-dates the patch.382 version bump and is **not in any release yet** — the shipped patch.382 `dist/rvf-backend.js` has zero `_maybeReloadFromDisk`, so the long-lived-reader staleness persists in production until a release carries this commit. Remaining gate = (1) cut a release including `01d1feac7`, then (2) convert this smoke to the `ACCEPT_TEMP` installed artifact (template: `smoke-adr0274-rvf-rw-split.mjs` + `scripts/lib/smoke-adr0255-shared.mjs`; the conversion only PASSES post-release). Do NOT claim this fix is "deployed"/"released": it is **implemented + fork-verified; awaiting a release.** The smoke is wired into the canonical harness (`run_check_bg` + `collect_parallel` in `scripts/test-acceptance.sh`; check fn `check_adr0274_xprocess_freshness` in `lib/acceptance-adr0274-checks.sh`).
