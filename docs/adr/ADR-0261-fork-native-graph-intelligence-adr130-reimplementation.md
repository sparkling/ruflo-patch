...
status: accepted
date: 2026-05-25
tags: [graph-intelligence, re-implementation, archivist, embeddings]
supersedes: []
...
* Good, because the "graph that forgets" capability lands as a real, audit-traceable surface that satisfies all 7 re-implementation criteria, with a bounded (~400-line) implementation surface and the schema in the right package (agentdb).
* Good, because A1 (coexist) keeps `causal_edges` stable controller-narrative data separate from mutable `graph_edges` memory-row data, avoiding the migration blast radius of [[ADR-0181]] Item 3 + [[ADR-0147]] R7 callers that A2/A3 would force.
* Good, because A-W1 reuses the archivist's existing audit-chain shape — no new signing pipeline, no new file dependency, and every graph-edge write stays audit-traceable.
* Bad, because coexisting `causal_edges` and `graph_edges` tables can create "which table do I query?" confusion (mitigated by distinct MCP tool names and an agentdb README documenting the split).
* Bad, because the quantized payload makes the `graph_edges` table grow fast (780B/edge for mpnet-768), requiring a nightly background sweep worker to hard-drop stale rows.
* Neutral, because Option B (extend `causal_edges`) was rejected on schema-impedance grounds and Option C (defer tighter) remains a valid fallback; this ADR is proposed-only until ratified, so ratification and implementation are separate steps.
...
Original status: accepted/completed 2026-05-25; revised, ratified, and implemented 2026-05-27. The two revision sections below record the council-ratification corrections and the port-to-upstream alignment that landed those dates.
