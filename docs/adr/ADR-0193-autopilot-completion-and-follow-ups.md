...
## Internal bugs surfaced + fixed during execution

(Neither of these came from `ruvnet/*` upstream — they're our own. Recorded here because they were latent / self-inflicted and caught only because the Item B probe forced an honest end-to-end signal.)

* **Build cascade trap (`scripts/build-packages.sh:296`) — pre-existing latent bug in `ruflo-patch`**: the `_af_has_dist == false` guard caused agentic-flow tsc to skip rebuild when dist already existed in `/tmp/ruflo-build`. Combined with rsync's `--filter='P dist/'` (preserves dest dist), fresh fork source NEVER reached published dist. Wave 1's release shipped Phase 2 src with Phase 1 dist. The `ctrl-autopilot-trajectories` probe correctly diagnosed as `trajectories=0 with episodes=4`. **Fixed in commit `931864f`** — tsc now always runs; `--incremental + .tsbuildinfo` for cache reuse; buildinfo invalidated when src is newer. The bug had been latent for an unknown duration; would have masked any future agentic-flow source changes too. Origin: pre-existing in ruflo-patch before ADR-0193 started.
* **Hook path resolver gap — self-inflicted by Wave 1 Agent 2 (commit `6a9d408`)**: that commit hardcoded `__dirname/../../agentic-flow/dist/...` in `autopilot-hook.mjs`. Right for monorepo dev; resolved to a triple-nested non-existent path in the installed `@sparkleideas/agentic-flow` package. **Fixed in commit `517e097`** — probes two candidate paths via `existsSync()`; first that exists wins. Origin: introduced by Wave 1's own Item C work during ADR-0193; caught by Wave 2's `ctrl-autopilot-stop-hook` probe in the same execution arc.

Both fixes have been exercised across 4+ release runs with no regression. No follow-up action required — recording the disposition + origin attribution closes the loop. Per memory `feedback-no-upstream-donate-backs.md`, neither gets reported to `ruvnet/*` even though one (the build cascade) is conceivably useful to upstream maintainers — fork improvements stay fork-only.
