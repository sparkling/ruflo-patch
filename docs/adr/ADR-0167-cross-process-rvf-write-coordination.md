...
### Amendment 2026-05-21b — Read-side reader convergence (warm-handle snapshot staleness): Phase 3

> **⚠️ RETRACTED 2026-05-22 — WRONG DIAGNOSIS.** This amendment misattributed `memory_search` returning `total:0` to RVF snapshot staleness. It was not. The real cause was a falsy-zero threshold coercion in the `memory_search` MCP tool (`memory-tools.ts`: `(input.threshold as number) || 0.3` turned an explicit `threshold:0` into `0.3`), a fork revision (rUv `eddfda0040`) that missed upstream's `?? 0.3` fix. `RvfBackend.search` was never broken — exact matches score 1.0, related content scores positive (verified live). The Phase-3 stat-staleness + reopen code (shipped in cli `patch.252`) was **reverted** (forks/ruflo `bf71e2bd3`); the real fix is `?? 0.3` (`a02e561ac`). See **Amendment 2026-05-22** below. The Phase-3 text is preserved verbatim per the ADR-0094 maintenance manifesto.

**Symptom.** On the long-lived MCP server, `memory_search` (semantic) returns `{ total: 0 }` for every query against an accumulated store, while `memory_store` / `memory_retrieve` (exact key) work and `memory_stats` reports full embedding coverage. The empty result returns in ~4 ms (empty-scan signature). A *fresh* process — or a `/mcp reconnect` that respawns the server — searches the same store correctly.

**Root cause — Phase 1 made *open-time* reads consistent; it never gave a *long-lived* reader a way to refresh.** Phase 1 (RootHeader, Amendment 2026-05-10f) fixed "what a reader observes *at* a write" — an `open()` gets a validated, non-torn manifest via `RootHeader::read → select_active`. But the native `RvfStore` loads `self.vectors` from the active manifest's VEC segments exactly once, in `boot()` (`forks/ruvector/crates/rvf/rvf-runtime/src/store.rs`, VEC-seg load ~`:2311-2323`), and `query()` is a **brute-force linear scan** over that in-memory map, returning empty iff `self.vectors.len()==0` (`store.rs:683`, guard `:694` — there is no HNSW graph in rvf-runtime). Nothing re-reads the RootHeader after boot. A handle held open across another process's commits is therefore frozen on its boot-epoch snapshot — and the upstream RVF reader protocol's standing requirement, *"To see new data: re-read Level 0 (explicit refresh)"* (`ruvnet/RuVector/docs/research/rvf/spec/09-concurrency-versioning.md`), was never implemented for a long-lived holder.

This surfaces in `memory_search` specifically because the fork routes the semantic read through the native store (ADR-0177 "adopt RVF vision" + ADR-0181 archivist `dispatchRead('memory_search') → vectorSearch`), wired to the **same** `RvfBackend` handle the writer uses (`cli/src/memory/archivist-init.ts:1497` `ensureRvfWired` wraps `getStorageInstance()`'s shared `_storage`). Because read and write share one handle, in-session stores ARE visible (so a fresh server works) — the staleness is **cross-process / cross-session / post-compaction**: the warm server booted its snapshot when the store was near-empty (the 2026-05-19 corruption + rebuild left it so), and entries that later accumulated via other processes are invisible to its frozen `self.vectors`. `memory_retrieve` / `stats` work because they read the TS `entries` Map (`@claude-flow/memory/src/rvf-backend.ts`), hydrated at boot and served independently of the native scan.

**Empirical confirmation (production `RvfBackend`, 2026-05-21).** A two-handle repro reproduces it deterministically: handle `H1` stores batch A (3); reader `R` opens (`R.count()==3`); a second handle `H2` appends batch B (3) and closes (on-disk = 6); `R` stays stale — `R.count()==3`, `R.getByKey('ns','b1')==null`, `R.search(b1)` returns only the 3 stale-A hits (b1 absent); a fresh reopen `R3` converges (`count==6`, finds b1). A fresh handle over a *copy of the live `.swarm/memory.rvf`* loads 40 entries and searches fine — proving the file and native path are healthy and the defect is purely warm-handle staleness.

**Why this belongs in ADR-0167.** It is the read-side complement of this ADR's cross-process coordination and builds directly on the Phase-1 RootHeader: the per-slot `txnid` is the exact, cheap staleness signal a long-lived reader needs (re-read the fixed-offset RootHeader, compare `txnid`, reload only when advanced — no tail scan). The distsys expert's Wave-2 framing ("what readers observe DURING a write", `:170`) extends naturally to "what a long-lived reader observes AFTER a write." Phase 1 = consistent reads at open; Phase 3 = consistent reads over a handle's lifetime.

**Evidence grounding (3-corpus, 2026-05-21).**
- **Upstream is no authority for the fix — and is *less* exposed than the fork.** Upstream's own `@claude-flow/memory` backend keeps a per-process JS `Map`+`HnswLite` as source of truth and treats native `.rvf` as **write-only** (only `nativeDb.open()`/`close()` are ever called; never `query`/`ingest`/read-back) — so upstream never reads from the shared native store and never hits this. The fork's native-READ path (`loadFromNativeSegments`, `nativeReverseMap`, the d8 self-heal) is **fork-only**, adopted to pursue the RVF substrate vision (ADR-0177); adopting native reads is what created the obligation to implement the substrate's explicit-refresh reader contract. The RVF spec is unambiguous: snapshot isolation, "re-read Level 0 to see new data," "snapshot valid for the fd lifetime even if the file is compacted and replaced" — and **no `refresh`/`reload` API exists at any layer** (Rust `RvfStore`, NAPI `RvfDatabase`, TS backend), confirmed by code survey.
- **No upstream issue matches this direction** (long-lived-empty / fresh-works) — it is fork-specific (a consequence of the fork-only native-read path). Closest siblings: upstream ruflo#1614 (write-side cross-process snapshot-overwrite; its shipped fix serialized only *intra*-process writes) and agentic-flow#129 (a wrapper holding a separate view from the inner native handle). The common upstream `memory_search=0` cluster (dim 768-vs-384, namespace defaulting; ruflo#1956) is a *different* root cause — ruled out here (store dim is consistently 768; a dim mismatch throws rather than returns empty).

**Decision — Phase 3: read-side reader-refresh, in the fork's TS layer (no native-core change).**

A 2026-05-21 reassessment (prompted by review) established that **extending the native core is unnecessary.** The native binding already exposes everything a refresh needs — `RvfDatabase.open` / `openReadonly` / `close` — and the empirical repro proved a fresh open converges (`R3` reopen → finds the new entries). RVF is snapshot-isolated *by design* and its spec delegates refresh to the **caller**; the fork is the caller, and the fork's native-read decision (ADR-0177/0181) lives in `@claude-flow/memory`, so the refresh belongs there — not in ruvnet's shared `rvf-runtime`/`rvf-node`. This avoids deepening divergence from upstream and a 5-target native rebuild.

1. **Staleness detection (pure JS).** In `@claude-flow/memory/src/rvf-backend.ts`, capture `{ino, size, mtimeMs}` from `fs.statSync(databasePath)` at load. Before a semantic `search()` / `query(semantic)` (folded into `ensureNativeSemanticReady`, `:1741`), `stat` again and compare: `size` grows monotonically on append (reliable even at coarse `mtime` granularity); `ino` changes on a compaction inode-swap. No change → no work (one `stat` per search).
2. **Refresh via reopen (existing native API).** On a detected change, reopen the native handle (`nativeDb.close()` + `RvfDatabase.open(databasePath)` — or maintain a dedicated `openReadonly` read handle to avoid any writer-lock churn) and **re-hydrate `entries` + `nativeReverseMap`** via the existing `loadFromNativeSegments` path (`iterAllWithVectors` / `listMetadataIds` / `getMetadataEntries`, `:2450`+). This *is* the substrate's required "explicit refresh," done from the consumer with zero native-core change. Update the cached `{ino, size, mtimeMs}` after reopen.
3. **Loud backstop (defense-in-depth, NOT a silent fallback — `feedback-no-fallbacks` compliant).** Extend the d8 self-heal (`rvf-backend.ts:751`) to ALSO fire when `raw.length === 0 && this.entries.size > 0`: a loud `console.warn` (stale-native-index detected; refreshed; recommend `ruflo memory rebuild` if it persists) + supplement via `pureTsSearch` / `bruteForceSearch` over the authoritative `entries` Map. Converting a silent wrong-empty into a logged correct answer is a correctness guarantee, not masking; the reopen (1–2) is the real fix, this covers the residual window.

**Deferred (optional, NOT implemented now): native in-place `reload`.** A native `RvfStore::reload()` + `root_txnid()` (re-read RootHeader, compare `txnid`/`file_identity`, re-apply in place) would be marginally more efficient (avoids a full re-boot; precise `txnid` signal instead of `stat`) and would benefit all native consumers (incl. agentdb's `RvfBackend`). It is a **pure optimization, not a correctness requirement** — `apply_manifest` re-reads all segments anyway, so at this store's scale (~40–60 entries) reopen is negligibly heavier. Deferred to a future ADR/Phase only if profiling shows a need; it MUST NOT block the TS fix or be conflated with it.

**Scope note.** The fix lands entirely in the fork's memory `RvfBackend` (the `memory_search` path). **No change to `rvf-runtime` / `rvf-node` (ruvnet core).** Phase 2 (δ-Rust coordinator, ADR-0168) composes with this — a single-writer daemon still needs readers to refresh.

**Test contract.**
- **JS regression test (primary; the gap that let this ship):** a behavioral `memory_search`-level test — a long-lived `RvfBackend` handle + a second handle that stores new entries; after a search the long-lived handle finds them (reader-refresh). This is the missing coverage — ADR-0090 B5 verifies persistence via `sqlite3` row-counts, NOT `memory_search`. Model on the verified repro (the `H1`/`R`/`H2`/`R3` sequence) but assert the *refreshed* long-lived handle converges. Resolve the freshly-built backend via `loadRvfBackend()` (ADR-0225), never a stale aggregate dist.
- **Acceptance:** store → (new `RvfBackend` instance on the same `.rvf`, simulating cross-process/restart) → `memory_search` returns > 0; and the live-repro scenario (warm handle + external append) returns > 0 after a `stat`-triggered reopen.

**Acceptance criteria (Phase 3).**
- **AC-P3-1:** JS reader-refresh regression test green via `loadRvfBackend()` (long-lived handle converges after an external write).
- **AC-P3-2:** live-repro scenario (warm handle + external append) returns > 0 after refresh.
- **AC-P3-3:** the loud `raw.length===0 && entries.size>0` backstop fires + warns (not a silent empty).
- **AC-P3-4:** **no change to `rvf-runtime` / `rvf-node`** — `git status` clean in `forks/ruvector`; no native rebuild required.
- **AC-P3-5:** acceptance preserved; no regression elsewhere.

**Implementation order** (TS-only; no cargo/native step):
1. `@claude-flow/memory/src/rvf-backend.ts`: add the `stat`-based staleness check + reopen-and-rehydrate (`ensureFresh`, called from `search` / `query(semantic)` / `ensureNativeSemanticReady`); guard new code so it no-ops when `nativeDb` is absent. `npm run build` the memory package.
2. Extend the d8 self-heal (`:751`) with the loud `raw.length===0 && entries.size>0` backstop.
3. JS regression test via `loadRvfBackend()` (the missing behavioral coverage).
4. Commit fork (`forks/ruflo` only) BEFORE release; then ONE `npm run release` end-to-end gate (acceptance + the new test). `forks/ruvector` is untouched.

Keeps ruvnet's core untouched, preserves all Phase-1 + d11–d14 invariants, adds no on-disk format change, no native rebuild. ADR-0167 status remains **accepted**; Phase 3 is a fork-TS-layer addition. ADR-0168 (Phase 2 δ-Rust) and the deferred native `reload` optimization are both unaffected/independent.

### Amendment 2026-05-22 — Phase 3 retracted; actual root cause = `memory_search` threshold falsy-zero

**Phase 3 (Amendment 2026-05-21b) was a wrong diagnosis and is reverted.** The `memory_search → total:0` symptom is **not** RVF snapshot staleness.

**Actual root cause.** `forks/ruflo/v3/@claude-flow/cli/src/mcp-tools/memory-tools.ts` computed `const threshold = (input.threshold as number) || 0.3;`. The `||` coerces a caller's explicit `threshold:0` ("no minimum") to `0.3`. Document embeddings score **0.2–0.5** cosine against realistic related queries (and negative for unrelated), so a 0.3 floor drops everything except near-exact (~1.0) matches → `total:0` for normal queries, and the swarm dialectic (which searches at the unset default 0.3) never recalls its own 0.2–0.4 memories.

**Evidence (live, 2026-05-22).** Same query, same store: `threshold:-1` (truthy → passes through) returned `authdoc-live` at similarity **+0.28**; `threshold:0` (falsy → coerced to 0.3) returned **0**; exact-content queries always returned similarity **1.0**. So the engine, embeddings (mpnet 768, provider `transformers.js`), dispatch, and namespace handling are all correct — only the threshold coercion was wrong.

**Why the misdiagnosis happened (lesson).** The investigation tested `RvfBackend.search` *directly* (the engine — which works), not the full MCP `memory_search` path (MCP tool → router → dispatchRead → handler), so the tool-layer coercion was never exercised until live store→search probes. **Test the actual MCP tool path first** (store + `memory_search` via MCP, and read raw scores with `threshold:-1`) before hypothesizing in the engine/storage layer.

**Fix — re-converge with upstream.** Upstream's same file/line uses `(input.threshold as number) ?? 0.3` (nullish): keeps 0.3 as the unset default, honors `threshold:0`. The fork carried an older rUv revision (`eddfda0040`, 2026-01-27) that predates upstream's fix. Restored `?? 0.3` (forks/ruflo `a02e561ac`). The agentdb engine convention corroborates this intent: `SemanticQueryRouter` uses `threshold ?? 0.0` ("0.0 for distance metrics") and `RvfBackend` applies `minScore` only when threshold is truthy (`0` = no floor). Default stays **0.3** (upstream value — not lowered; lowering would diverge). Callers wanting recall pass `threshold:0` (now honored). INTEGRATION-LEDGER row added for the re-convergence.

**Net.** Phase-3 reader-refresh reverted (`bf71e2bd3`); threshold fix applied (`a02e561ac`); no `rvf-runtime`/`rvf-node` change (was never needed). The `op.minConfidence || 0.3` siblings at `memory-router.ts:2064,2095` are the same falsy pattern in the *pattern-search* path — noted as a follow-up, not changed here (out of the observed scope; no upstream reference to re-converge against).

### Correction (2026-05-22b): the threshold story above was ALSO a misdiagnosis — real cause is RVF metric-not-persisted (see ADR-0073 amendment)

The "Actual root cause" / "Fix — re-converge with upstream" sections above (the `|| 0.3` → `?? 0.3` change) were a **second wrong layer**. The `?? 0.3` change is harmless and a legitimate upstream re-converge, but it **neither caused nor fixed** the `total:0` symptom.

- **The "0.2–0.5 cosine" / "+0.28" evidence was misread.** `memory_search` does not return raw cosine — on a reopened store it returns **`2cos − 1`**. The `+0.28` hit was simply more-similar content (true cos ≈ 0.64 → `2·0.64−1 = 0.28`); realistic related content (true cos 0.3–0.5) scores **negative** (`−0.4 … 0`). So `threshold:0` does **not** recall it (negative `< 0`) — the `?? 0.3` fix only helps the `[0, 0.3)` band, not the realistic case.
- **Actual root cause:** RVF's native `open()` does not persist the distance metric (`RvfStore::try_open_once` → `..Default::default()` → L2). A store created `cosine` reopens as `l2`; its query returns `‖a−b‖² = 2−2cos`; the ADR-0073 `1 − r.distance` cosine conversion turns that into `2cos − 1`; the 0.3 gate then drops everything. Full analysis + the durable fix (score cosine **directly**, metric-independent) are in the **ADR-0073 amendment (2026-05-22)**.
- **Lesson reinforced:** the live probe *was* the right move, but the score scale was taken at face value. Read raw scores AND compare them to an independent cosine (`embeddings_compare`) — the `2cos−1` offset is only visible by that comparison, not by reading `memory_search` alone.

### Threshold stance superseded (2026-05-22b → ADR-0227)

The "keep the 0.3 floor (upstream value); callers pass `threshold:0` for recall" decision in the **Fix — re-converge with upstream** section above is **superseded by ADR-0227**. Measured mpnet cosine: related content ~0.25–0.65, unrelated ~0 — so 0.3 cuts into real recall (and the fork's own agents/checks don't pass `threshold:0`). ADR-0227 lowers the fork-only adaptive ONNX floor 0.3 → 0.15 (the hardcoded fallback stays 0.3, still mirroring upstream's code value). The metric-not-persisted findings in this ADR's earlier amendments are unaffected.
