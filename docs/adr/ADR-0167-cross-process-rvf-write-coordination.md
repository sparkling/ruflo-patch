...
**Test contract (was unpinned — how the gap shipped).** New regression test asserts: pure-TS `loadFromDisk` MUST NOT throw `corrupt`/`bad magic` for files with native magic (`SFVR`, full `RVFROOT\0`, OR a bare `RVFR` prefix); AND MUST still fail-loud for genuinely-corrupt non-native pure-TS files (ADR-0090 preserved). See `tests/unit/adr0167-loadfromdisk-native-magic.test.mjs`.
