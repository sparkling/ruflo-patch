...
| `1117b66` → `1767ef8` (revert) | L11 REVERTED 2026-05-17 | The Wave 3 baseline release (commit `8bc65d7`) surfaced 2 latent bugs that broke L11: (1) `_acceptance_snapshot` returned non-zero when copying `$E2E_DIR/.claude-flow/daemon.sock` (BSD `cp -cR` can't copy sockets); the L11 check used `_acceptance_snapshot $E2E_DIR $iso \|\| return` and tripped on that. (2) The original L2 helper had a separate dst-already-exists bug (fixed in `f8d3479`) that masked this socket-tolerance issue from earlier targeted tests. Pre-requisites for re-attempting L11: (a) update `_acceptance_snapshot` to tolerate cp's non-zero exit on socket/lock/wal files (mirror L10-ext's narrowing for volatile-file races), AND (b) audit L11 check sites to verify they don't rely on snapshot exit code |
| `34cbf58` → `bb14830` (revert) | L13(b) REVERTED 2026-05-17 | Wave 3 baseline release surfaced that adr0177's 2 default-mpnet checks run in group `p5-cfg-valid` where `$E2E_DIR` isn't in scope (empty env var in the check's subshell — observed `[L10-ext][error] _e2e_isolate: $E2E_DIR/.claude-flow missing` with `$E2E_DIR=` empty). My L13(b) refactor assumed `_e2e_isolate` is available in all check phases — wrong. Pre-requisite for re-attempting L13(b): audit the phase scheduling in `scripts/test-acceptance.sh` to identify which check groups have `$E2E_DIR` exported, and either (a) move adr0177 to a group that has it, or (b) skip L13(b) for checks in pre-E2E phases |
