...
| L11 | Replace init-incidental per-check inits with clone-from-golden (`cp -cR $ACCEPT_TEMP/. $check_dir/`) or `.claude-flow/config.json` mutation | Drops ~3–5 of 17 per-release `init --full` invocations (~1.0–1.7k file writes saved/release) | The pipeline runs 17 distinct `init --full` invocations per release (harness × 1, adr0088 × 2, adr0098 × 3, adr0100 × 1, adr0104 × 1, adr0177 × 4, adr0178 × 1, adr0143-init-mcp × 1, hive-mind × 2, phase10 × 1). **9 are init-as-SUT** — they test init's own code paths under specific conditions (PATH probing in adr0088, prior-state in adr0098, cwd handling in adr0100, wrapper bin in adr0143-init-mcp, idempotency in phase10, dim auto-adjustment in adr0177 MiniLM) and cannot be reduced. **The remaining 8 are config-outcome tests** where init is the easy setup, not the SUT. Of those, **3 are clone-from-golden safe** (adr0104, hive-mind × 2 — they just need a fresh post-init project) and ~2–5 are config-mutate candidates (adr0177's 2 default-model inits and adr0178's `--with-embeddings` invocation — per-check audit required to confirm which assertions are downstream-of-config vs init-itself). The pattern generalises the existing `_e2e_isolate` model (which already clones node_modules via symlink) to the full post-init project state. **Honours `feedback-no-fallbacks`:** if a clone breaks because the check relied on init side-effects (daemon socket creation, fresh RVF seeding from cold), the check fails loud and runs its own init in a follow-up commit — no silent fallback. Lands *after* L2 since it reuses the `_acceptance_snapshot()` `cp -cR` helper |
