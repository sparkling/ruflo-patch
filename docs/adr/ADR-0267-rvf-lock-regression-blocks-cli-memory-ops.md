...
## Amendment: resolved by ADR-0274 (2026-05-30)

**Resolved.** ADR-0274 (read/write handle split, single-handle park/unpark model) shipped and was released (`@sparkleideas/*` patch.376+). The MCP server no longer holds the RVF `flock` for its lifetime: it parks (releases the flock) when idle and re-acquires only for the duration of a write transaction (debounced on a 50 ms idle timer; lightweight manifest-only resync on re-acquire). The `adr0274-rvf-rw-split` acceptance smoke — which forces RVF warmup via a real `tools/call`, then runs a concurrent CLI `memory store` from a separate process — is green (no `LockHeld`, no 30 s hang). Demonstrated in production: the 281-ADR corpus index (ADR-0271 Phase 3) was built via `agentdb index` alongside a live MCP server with no contention. The idle-only Revision 3 fix this ADR previously carried was insufficient (it only covered the never-warmed server); ADR-0274 is the general fix. This regression tracker is closed.
