...
**Status: DECLINED 2026-05-29 (see [[ADR-0270]]).** Not implemented, and will not be. Three reasons: (1) **Wrong preset** — team coordination (Agent-Teams: `teammate_mailbox`, `team_context`, `@name`, task auto-claim) is collaboration-during-execution, which is what **swarm** is for; the Council/Consensus hive patterns depend on the *opposite* — independent, one-round, mutually-uninfluenced worker stances the queen composes (see `templates/worker-contract.md`: "you will not see peer replies"). Binding hive workers to a team injects the cross-worker visibility that breaks that dialectic. (2) **Already in swarm** — `plugins/ruflo-swarm/skills/swarm-init/SKILL.md:16` already does `TeamCreate` + `Agent` + `SendMessage`; Piece 6 was duplicating a swarm capability onto the wrong preset. (3) **Shared surface** — the chosen Option C mutates the `hive-mind_*` MCP return shape, shared with the base hive-mind skill + upstream. The swarm-side twin [[ADR-0146]] is likewise declined + un-parented. The original Piece 6 proposal (Options A/B/C + contract) is preserved below for history.
...
- **Piece 5 — handler-invocation tests for `_join/_leave/_broadcast/_memory` (NEW PIECE).** Per Finding 2, these tools lacked dedicated tests in `cli/__tests__/mcp-tools-deep.test.ts`. **Executed 2026-05-29** (fork `e69683975`; see [[ADR-0270]]): added 6 `it()` blocks (happy + fail-loud guard) for `_join/_leave/_broadcast` in the existing "Hive Mind Tools - Handler Invocation" describe. `_memory` was found to be already covered by the ADR-0122 (T4) / ADR-0123 (T5) / ADR-0131 (T12) blocks, so Finding 2's "four tools" was itself stale — the real gap was three. Verified against the handler source contracts + the existing passing idiom; authoritative green is the CI/release vitest run (the suite is build-tier — it requires the installed workspace to resolve `agentdb/archivist`, so it does not load in the raw dev tree).
