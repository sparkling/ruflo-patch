...
2. **`PostgresBackend.ts` retention.** Museum code or full removal? Lean: retain as `// @deprecated` reference for the future @ruvector PG extension hook; remove during a later cleanup pass.
...
9. **Federation + RAFT distributed mode (deferred).** Upstream ADR-007 Phase 5 includes distributed deployment. Single-node first per `feedback-no-fallbacks` discipline; multi-node revisit if/when a deployment scenario justifies it.
