...
### New checks (10 scenarios — `lib/acceptance-adr0104-checks.sh`)

| ID | Section | Asserts |
|---|---|---|
| `adr0104-mcp-path` | §4a | `.mcp.json` claude-flow command uses direct path when in `$PATH`; falls back to `npx -y` otherwise |
| `adr0104-obj-required` | §2 | `hive-mind spawn --claude` (no objective) exits non-zero with "Objective is required" |
| `adr0104-obj-via-flag` | §2pos | `-o "obj"` preserved in generated prompt file |
| `adr0104-noninter-global` | §1 | `--non-interactive "obj"` parses non-interactive=true AND obj as positional (parser hoist) |
| `adr0104-no-1422` | §6neg | Generated prompt does NOT contain the upstream `#1422` "Do NOT use Claude native Task/Agent tools" block |
| `adr0104-v3-contract` | §6pos | Prompt contains `🛠️ TOOL USE` + `📝 WORKER COORDINATION CONTRACT` blocks; 4-phase PROTOCOL preserved |
| `adr0104-meta-preserved` | §6meta | Prompt parameterization (Queen Type, Topology, Consensus, Worker Count, Worker Distribution) preserved |
| `adr0104-honest-wording` | §3 | Spawn output says "Registered N worker slot(s)" with "actual worker" note; no "Spawned N agent(s)" |
| `adr0104-mem-distinct` | §5 | 8 parallel `hive-mind memory set` with distinct keys → all 8 entries persist; lock cleaned up |
| `adr0104-mem-same-key` | §5 | 8 parallel writes to same key → JSON well-formed, exactly one writer-* value persists; lock cleaned up |

Wired identically to ADR-0098: source line in the lib block (~L520), `run_check_bg` block (~L1280), `_adr0104_specs` array, expansion in the wait-loop. Gated on `$E2E_DIR/.claude/settings.json` so a non-init'd accept dir doesn't 10x-fail.

### Paired unit test — `tests/unit/acceptance-adr0104-checks.test.mjs`

35 tests across 5 describe blocks, all green:

- **Static structure** (12): lib file exists; all 10 check functions defined; every check sets `_CHECK_PASSED`/`_CHECK_OUTPUT`.
- **Runner wiring** (12): `adr0104_lib` sourced; all 10 check IDs registered with `run_check_bg`; `_adr0104_specs` expanded in the wait list.
- **§1 parser hoist** (2): `non-interactive` global boolean entry exists in compiled `parser.js`; behavioral test imports `CommandParser`, registers stub `hive-mind spawn`, asserts `parse(['hive-mind','spawn','--non-interactive','Build a REST API'])` produces `flags.nonInteractive===true` AND `positional===['Build a REST API']`.
- **§5 lock under contention** (4): `withHiveStoreLock` + `O_EXCL` + atomic rename present; `set`/`delete` route through lock, `get`/`list` bypass; behavioral N=8 distinct-key test (all keys persist); behavioral N=8 same-key test (one winner, JSON intact, lock cleaned).
- **§6 prompt content + §3 wording** (3): TOOL USE block present, `#1422` block absent, 4-phase PROTOCOL preserved, "Registered N worker slot(s)" wording present, no "Spawned N agent(s)".
- **§4a mcp-generator** (2): `detectClaudeFlowPath` + `createClaudeFlowEntry` exported; `mcpServers['claude-flow']` uses `createClaudeFlowEntry` (not raw npx wrapper).

Suite tally: 3631 → 3666 (+35), 0 failures, 13 skipped (unchanged allowlist). ADR-0097 lint clean (0 errors / 0 warnings across 80 files).

### Source changes (fork)

| File | Change | Section |
|---|---|---|
| `v3/@claude-flow/cli/src/parser.ts` | Add `non-interactive` to `globalOptions` boolean set | §1 |
| `v3/@claude-flow/cli/src/init/mcp-generator.ts` | `detectClaudeFlowPath()` + `createClaudeFlowEntry()` helpers; `mcpServers['claude-flow']` uses direct path when present | §4a |
| `v3/@claude-flow/cli/src/mcp-tools/hive-mind-tools.ts` | Atomic write in `saveHiveState`; `withHiveStoreLock` helper (O_EXCL + stale recovery, lifted from ADR-0098); `hive-mind_memory` set/delete wrapped under lock | §5 |
| `v3/@claude-flow/cli/src/commands/hive-mind.ts` | Replace `#1422 TOOL PREFERENCE RULES` block with `🛠️ TOOL USE` + `📝 WORKER COORDINATION CONTRACT`; existing 4-phase PROTOCOL + parameterization headers preserved | §6 |

§2 (hard-error on missing objective) and §3 (honest "Registered worker slot(s)" wording) were already applied to fork on 2026-04-28 prior to this work — paired tests now lock them in.

### Deviations from ADR text

- ADR §4a wrote `claudemd-generator.ts`. Q3 verification showed `.mcp.json` is generated by `mcp-generator.ts`. Implementation redirects to the actual location per Q3's mitigation ("If separate generator: redirect §4a edits there. ~15 min code read at implementation start.").

### Out of scope (per ADR §"Out of scope" — preserved as deferred)

Q1 empirical verification (Task sub-agent MCP access) and §4a empirical verification (post-build MCP attach status) require a real `claude` CLI session — manual smoke test (`scripts/manual-hive-smoke.sh`) per the ADR. Not wired into automated acceptance. ADR-0094 coverage tracker stays "100%-of-checkable surface"; the smoke test is documented in CLAUDE.md per the ADR's intent.
