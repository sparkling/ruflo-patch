...
- **Content-preserving transform — no summarization (HARD RULE).** The conversion *relocates* existing body content under MADR headings; it MUST NOT condense, paraphrase away, or drop it. Permitted edits: add YAML frontmatter (from the old status/date block), strip the `ADR-NNNN:` H1 prefix, rename/insert the required MADR headings, move existing prose/tables/code blocks verbatim beneath them, reformat consequence bullets to `* Good/Bad/Neutral, because …`, and lift body-prose typed relations into frontmatter. Every substantive line of the original must survive somewhere in the converted file. A first run free-rewrote and summarized the long ADRs (e.g. ADR-0020 testing-strategy: 406 → 90 lines, dropping the 3-layer test taxonomy and the 78-test inventory table) — that is a **defect**, not an acceptable trade.
- **Fidelity gate (enforced).** After converting a file, compare its word count to the original (`git show HEAD:<path>`). The converted file MUST retain **≥ 95%** of the original word count. Reformatting (status block → frontmatter, `### Positive` → `* Good, because`) costs only a few words; a larger drop means content was summarized → the file FAILS the gate and is redone. The gate runs per file (`/tmp/adr-fidelity.mjs`) alongside the conformance validator; both must pass before a file is considered done, and the whole corpus is re-checked before commit.
