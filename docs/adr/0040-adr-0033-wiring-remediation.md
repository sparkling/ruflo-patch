...
### Issue 1: Non-exported classes (6 controllers)

HierarchicalMemory, MemoryConsolidation, MutationGuard, AttestationLog, GuardedVectorBackend, and SemanticRouter are not in `agentdb/src/index.ts` exports. All fall back to stubs or null. The security layer (MutationGuard + AttestationLog + GuardedVectorBackend) is entirely non-functional.
