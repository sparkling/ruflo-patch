...
## Amendment (2026-05-22): the metric is NOT persisted on reopen — score cosine directly

**Defect introduced by this ADR's Phase-3 "Metric-aware score: cosine `1-d`".** That conversion is correct only if the store's actual metric is cosine. It is keyed off `this.config.metric` (always `'cosine'` for memory), **not** the store's on-disk metric — and the two diverge on every reopen.

**Root cause (RuVector rvf-runtime, upstream-shared).** RVF does not persist the distance metric: `RvfStore::try_open_once` rebuilds `RvfOptions { domain_profile, ..Default::default() }`, and `RvfOptions::default().metric = DistanceMetric::L2`. The metric is not in the persisted manifest at all, so `open()` cannot restore it. **A store created with `metric:'cosine'` reopens as `l2`.** (Identical in `ruvnet/RuVector` — `store.rs:149/195` — so it is a longstanding upstream invariant, not a fork regression.)

**Consequence.** For the unit-normalized mpnet embeddings (ADR-0069), an `l2` query distance is `‖a−b‖² = 2 − 2cos`. The `1 − r.distance` cosine formula then yields **`2cos − 1`** (negative for merely-related content). The `memory_search` 0.3 threshold gate (ADR-0004 contract) drops everything → **`total:0`**. Verified live (2026-05-22): `embeddings_compare` reports cos `+0.38`/`+0.28` for related pairs while reopened-store `memory_search` returns `−0.24`/`−0.44` (`= 2cos−1` exactly); driving the installed `ruvector-rvf-node@0.1.7-patch.130` directly returns `1−cos` on the create handle but `l2` after `open()`.

**Why it stayed hidden across millions of installs.** For unit vectors L2, cosine, and IP are **rank-equivalent** (`2−2cos` is monotonic in cos) — verified: identical top-k order. ANN nearest-neighbour retrieval (the dominant use) is therefore correct regardless of the metric default; only the *absolute* score value (and a threshold gated on it) is wrong. Upstream `agentic-flow`'s `ruvector-backend.ts` and `ruvnet/ruflo`'s memory backend both compute/return a **similarity score directly** (never converting a raw distance), so they never depend on the non-persisted metric. Our ADR-0073 migration to the *distance*-returning binding is the only path that does. Note also: a fresh CLI `memory search` reopens with an empty `nativeReverseMap` and falls through to `pureTsSearch` (correct cosine); the defect surfaces in long-lived processes (the MCP server / daemon) whose native ANN path runs against the reopened `l2` handle.

**Decision — compute cosine directly (re-converge with upstream's pattern).** `RvfBackend.search` now scores via `cosineSimilarity(queryVec, entry.embedding)` instead of `1 − r.distance` — metric-independent, exact, and identical to `pureTsSearch`/`bruteForceSearch` and upstream `agentic-flow`. Native ANN still drives candidate *ranking* (rank-equivalent for unit vectors); we re-score only for the absolute similarity the threshold compares, then sort score-DESC before the top-k slice. No `rvf-runtime`/`rvf-node` or RVF-format change: persisting the metric would touch a longstanding, ranking-irrelevant upstream invariant for no benefit to ANN consumers. `forks/ruflo` `src/rvf-backend.ts` (native search loop). Provenance: agentdb was spun off from agentic-flow (`ruvnet/agentdb` is a 2026-05-06 snapshot, 7 commits); compare RVF/vector-backend behaviour against `agentic-flow`, not the agentdb snapshot.
