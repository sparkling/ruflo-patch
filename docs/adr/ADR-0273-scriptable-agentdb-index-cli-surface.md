...
**D9 — Write edges to RVF now; do not wait for ADR-0147 R7.** Edges persist to RVF today (the controller arm is dead — `CausalMemoryGraph.addCausalEdge` needs numeric IDs the string-keyed surface lacks; R7 unimplemented). ADR-0274 removes the only obstacle (the lifetime flock), so the index writes edges to RVF contention-free now. R7 will later move them to SQLite transparently; gating the index on R7 would block it on unrelated cross-package infrastructure. **Update 2026-05-30: that "later" is now [[ADR-0276]]** — it re-converges these edges from the KV `causal-edges` namespace onto the `CausalMemoryGraph` controller (numeric IDs via a durable string→numeric map), reusing this command's `--purge` rebuild as the migration carrier. The index keeps writing edges; ADR-0276 only redirects *where* they land.
...
- **Depended-on-by [[ADR-0276]]** (2026-05-30): re-converges this command's causal-edge writes from the KV `causal-edges` namespace onto upstream's `CausalMemoryGraph` controller, with the durable string→numeric ID map ADR-0147 R7 named. The `--purge` rebuild here is ADR-0276's migration carrier; the FROM→TO key-collision fidelity caveat (10 edges / 5 ADR pairs lost to KV upsert) is dissolved once edges are controller rows keyed `(from_memory_id, to_memory_id, relation)`.
