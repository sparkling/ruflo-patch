...
* **Performance and hot paths.** The caller catalog identifies two HOT-path writers that fire on every Claude Code lifecycle event: `post-edit` hook (every Edit/Write/MultiEdit, currently `fs.appendFileSync` to `pending-insights.jsonl` with a <2ms budget) and `pre-task` hook (every Agent invocation, currently a cheap counter increment). Full archivist ceremony per call (guard + audit + post-write triggers) is incompatible with these budgets. The archivist exposes a **fast-path** for registered hot writers:
  * Audit entries are emitted to an in-memory ring buffer + batched fsync at most every 100ms (durable on shutdown via `process.on('beforeExit')`).
...
| 7 | Hooks + daemons | star | post-edit-migrator (hotPath:true, backend-dev); pre-task-migrator (hotPath); post-task-migrator; session-end-migrator (includes consolidation-nudge wiring + recovery-scan bounds per #11); 6× daemon-worker-migrator (one per scheduled worker — consolidation, auto-memory, hooks-learning, sync, autopilot, ranked-context); fail-loud-fixer (analyst, #14 sites); hot-path-bench-validator (tester, W3 + W3-contended real-pipeline fixture) | task-orchestrator | 12 + 1 | W3 microbench (single-process) passes; W3-contended real-pipeline (cli + daemon + Edit storm) passes; consolidation-nudge acceptance test passes; recovery-scan cap test passes |
...
**Up to 15 agents per phase** — the table above shows where the parallelism actually fits. Phases 5 and 6 hit the 15-agent ceiling; phases 2, 7 sit at 7-12; phases 3, 4, 8, 9, 10 sit at 4-10. Single-agent phases were ruled out — every phase has at least 4 workers + 1 queen because the queen-alongside-workers pattern (per `feedback-council-queen-da-alongside-experts`) requires concurrent spawn.

**Pre-Phase-2 prerequisite (no swarm needed).** The `scripts/ruflo-publish.sh` pipeline gains the `ADR-0180-Halt:`/`ADR-0180-Amendment:` trailer scan + the charter conformance hook before Phase 2 spawns. This is a one-shot scripting task (sole-committer ruflo-patch repo), not a swarm phase. Without it, the halt protocol doesn't bite.

**Phase 0 (audit gates, prior to execution).** This ADR's acceptance status flip (`proposed` → `accepted`) is itself the Phase 0 gate — until accepted, no implementation phase runs. The two audit passes (Pass 1 + Pass 2) documented in this document are the artifacts of Phase 0; a third pass after the Execution Plan landing is optional but recommended before accepting.
...
13. **Hot-path fast-path concrete budget.** §Performance lists `post-edit` and `pre-task` as the two HOT hook writers. The fast-path budget (in-memory ring buffer + batched fsync ≤100ms, guard skipped, async post-write triggers) needs a concrete latency target (per-call median + p99) before Phase 7. Current `pending-insights.jsonl` `appendFileSync` is <2ms; the archivist fast-path must stay under that ceiling.
    ...
    * **p50 ≤ 300μs** — covers ring-buffer push, `MutationContext` allocation, audit-entry shape (`{ auditId, originatingTool, processId, parentAuditId?, timestamp, payloadHash }`), and JSON serialization of a typical hook payload (≤2KB). No I/O.
    ...
    *Async drain SLO (separate from latency budget):* fsync drain runs every ≤100ms (§Performance). Buffer-to-disk freshness window is p99 ≤ 100ms; entries are durable on `beforeExit`. Drain latency is NOT charged to the hot-path budget — drain is off-path by construction.
    ...
    *Hot-path registration contract (`registerMutationHandler(..., { hotPath: true })`):* By opting in, the handler asserts:
    ...
    2. **No synchronous fsync.** Audit entry enqueues to the ring buffer only; fsync coalesces in the async drain.
    ...
    5. **Bounded payload size.** Payloads >4KB fall back to cold-path automatically (runtime check at ring-buffer enqueue diverts oversized intents). Hook writers expecting larger payloads (e.g., session-end's consolidation outputs) must register WITHOUT `hotPath: true`.
    ...
    *Out of scope.* Cold-path budgets (consolidation, session-end, sync — full ceremony acceptable at any latency under ~50ms p99 per write). Read-path budgets (reads have no audit; most are passthroughs). Backpressure overflow when the ring buffer fills faster than drain — see follow-up #17.
...
15. **Multi-process audit semantics.** The cli runs in one process; `ruflo daemon` runs in another; both write to the same substrate. The archivist is per-process — how do their audit chains compose? Options: (a) shared append-only audit-log file with file-locking ordering; (b) per-process audit logs that merge by timestamp at replay time; (c) audit chains are per-process and replay is per-process too (the user picks which to replay). Load-bearing for Phase 2 because the daemon's scheduled writers will write through *some* archivist instance; the ADR doesn't say which.
    ...
    *Watchdog mechanisms (defense-in-depth):* **(W1) Grep gate** runs `grep -RE 'MutationContext.*child\(.*\bRPC\b|enqueueWith.*auditId|parentAuditId\s*:' forks/<archivist-package>/src/` in `npm run release`'s preflight stage (`scripts/ruflo-publish.sh`). Any match blocks release unless the head commit carries an `ADR-0180-Halt: cross-process-mcc` trailer with a paired Addendum commit. The same grep also runs in `npm run test:unit` for fast feedback during phase work. **(W2) Runtime warning:** `archivist/mutation-context.ts` emits a one-shot WARN-level log `"cross-process MutationContext.child() invocation — revisit ADR-0180 #15"` if any `child()` call site is detected as serializing the context (heuristic: presence of a `serialize()`/`toJSON()` invocation on a `MutationContext` instance). Operational visibility for cases the grep misses.

    The intent of (R1-R4) + (W1) + (W2) is not to block the deferral — it is to ensure deferral never silently lapses into "we forgot to revisit". W1 is automated gating in the existing release pipeline; W2 is runtime evidence; the Halt-trailer is the deliberate-escalation path.
...
19. **Replay test harness — who, where, how often.** §Confirmation says "replay the audit log against a freshly-initialized substrate MUST yield ... addressable-key set-equality." But: who writes this test (per-handler? per-surface? top-level acceptance?)? Where does it live (`forks/<archivist-package>/test/`?)? How does it map onto the existing two-command gate surface (`npm run test:unit` for fast feedback vs `npm run release` for full pipeline) — given replay against 200+ call sites' worth of audit entries may be slow? Decide before §Confirmation is shippable as an acceptance gate.
    ...
    *Out of scope.* Per-handler replay-equivalence assertions (handlers tested via #20's `withTestContext`); cross-process MutationContext.child() replay across cli→daemon boundary (deferred per §15 — trigger-bound; re-opens when R1-R4 fire; W1 + W2 watchdogs prevent silent lapse); HNSW-topology equivalence (excluded by §Confirmation).
