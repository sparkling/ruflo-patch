...
**Phase 9 inter-controller orchestrators — partial (2026-05-28).** ADR-0181 closed without executing Phase 9 (the NightlyLearner / MemoryConsolidation / SkillLibrary / SyncCoordinator re-entrancy cascades named in §Re-entrancy + §Caller surfaces), so it is tracked here. First piece landed: `NightlyLearner.run()` now drives `SkillLibrary.consolidateEpisodesIntoSkills` (agentdb `ebf73a5`), restoring the episode→skill promotion folded in from [ADR-0179](ADR-0179-restore-controller-instrumentation-lost-in-adr0085-bridge-deletion.md) (row 6 — `superseded-by` this ADR). It mints `ctx.child('skill')` per §Re-entrancy but writes via SkillLibrary's legacy SQLite path until an F4-3 caller mints a root `MutationContext`. **Residual (open Phase 9):** (1) autonomous episode recording — no hook/daemon populates `episodes`; `storeEpisode`/`agentdb_reflexion-store` is caller-driven; (2) scheduled `run()` invocation (today on-demand via `agentdb_learner_run`); (3) F4-3 substrate-seam routing of the consolidation writes. These keep "automatic feedback→skill promotion" **mechanism-complete but not yet fully autonomous**. The residual's execution vehicle is now [ADR-0268](ADR-0268-autonomous-skill-promotion-flywheel-adr0180-phase-i.md) — the autonomous skill-promotion flywheel (record-inline + batch-promote + retrieve-pre-task, `task_type`-keyed), which realizes this Phase 9 residual and fulfills the upstream `LEARNING-OPTIMIZED-PLAN` vision `ADR-053` deferred.
