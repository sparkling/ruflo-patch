...
ruflo tracks upstream `claude-flow`/`ruflo` but applies our own patches and enhancements. The version number must communicate two things:
...
Our package name (`ruflo`) is different from the upstream package names (`ruflo`, `claude-flow`, `@claude-flow/cli`), so we are not competing for the same version space on npm.
...
Use the format `{upstream_version}-patch.{N}` for all ruflo package versions.

Examples:

| Event | Upstream Version | ruflo Version |
|-------|-----------------|---------------------|
...
- Our package name (`ruflo`) is different from upstream (`ruflo`/`claude-flow`) — there is no version space collision
- Within our own package, versions sort correctly: `3.5.2-patch.1 < 3.5.2-patch.2 < 3.5.3-patch.1`
- `npm install ruflo@latest` resolves to whatever we've tagged as `latest`, regardless of semver sorting
...
1. **Independent versioning (1.0.0, 1.0.1, ...)** — Rejected. Loses the connection to the upstream version entirely. Users cannot tell what upstream code they're running. "ruflo@1.4.7" tells you nothing about whether it includes upstream 3.5.2 or 3.6.0.
...
- `npm outdated` comparisons are meaningful within the ruflo package: `3.5.2-patch.1` is clearly older than `3.5.2-patch.3`
...
- If we need to unpublish a bad version, `npm unpublish ruflo@3.5.2-patch.2` removes exactly that version. The previous `3.5.2-patch.1` remains available
...
The ruflo ecosystem rebuilds packages from multiple upstream repos that have independent version numbers:

| Upstream Repo | Upstream Version | Our Package | Our Version |
|---------------|-----------------|-------------|-------------|
| `ruvnet/ruflo` | 3.5.2 | `ruflo` | `3.5.2-patch.1` |
...
- [ ] `npm view ruflo versions` shows correctly ordered versions
- [ ] `npm install ruflo@3.5.2-patch.1` installs the exact specified version
- [ ] `npm install ruflo@latest` installs the promoted version, not the highest semver
- [ ] Internal `@sparkleideas/*` packages from the `ruflo` repo use the same version as `ruflo`
