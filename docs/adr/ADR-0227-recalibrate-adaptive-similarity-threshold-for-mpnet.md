...
# Recalibrate the adaptive similarity threshold for mpnet (0.3 → 0.15)

> **Supersession / relates (template normalised 2026-05-22 — YAML frontmatter
> added, `ADR-0227:` H1 prefix removed per cohort convention).** Supersedes
> ADR-0004 (lower-to-0.1, long-obsolete) fully, and ADR-0167's "keep the 0.3
> search floor" stance only — ADR-0167's metric-not-persisted amendment
> stands (so 0167 is not fully superseded; it stays out of the bare
> `supersedes:` list). Relates: ADR-0069 (mpnet unified model), ADR-0073 (RVF
> cosine-direct scoring), FB-004 (`getAdaptiveThreshold`). **Verified shipped
> (direct review 2026-05-22):** `embedding-adapter.ts:205` returns `0.15` for
> ONNX (was `0.3`); MCP `memory-tools.ts` routes through `getAdaptiveThreshold`
> (the `?? 0.3` hardcode removed from the MCP path). Fork `16d7c87cd`,
> `@sparkleideas/cli` patch.259.
