...
If a version promoted to `@latest` on npm turns out to be broken, users running `npx ruflo-patch` will get the broken version. npm does not support "unpromoting" a dist-tag -- `latest` is simply a pointer to a specific version. The only way to stop users from getting a broken version is to point `latest` at a different (known-good) version.
...
```bash
npm view ruflo-patch versions --json | jq -r '.[-2]'
```
...
This takes effect immediately. Users who run `npx ruflo-patch@latest` after this point get the known-good version. No cache busting is needed -- npm dist-tags are resolved server-side on every install.
...
1. Confirm the broken version: `npx ruflo-patch@latest --version` shows the bad version
...
4. Verify: `npm view ruflo-patch dist-tags` shows `latest` pointing to the good version
5. Verify: `npx ruflo-patch@latest --version` shows the good version
