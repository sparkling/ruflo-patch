...
status: accepted
date: 2026-03-05
tags: [systemd, ci, pipeline, automation]
supersedes: []
depends-on: []
implements: []
---

# systemd timer for automated builds
...
* **Use a systemd timer and a oneshot service to trigger the build script on a 6-hour cadence — no external CI system (chosen).**
* **GitHub Actions self-hosted runner** — Rejected. Self-hosted runners now cost money. Our server is already running and more powerful than any GitHub-provided runner. Adding a runner agent is an unnecessary dependency on GitHub infrastructure.
* **GitHub Actions hosted runners** — Rejected. Costs money per minute. Our 32-core server builds TypeScript in minutes. Paying for cloud compute we already own makes no sense.
* **Woodpecker CI** — Rejected. Another service to install, configure, update, and monitor. Overkill for a single linear pipeline that a bash script handles.
* **Forgejo + Actions** — Rejected. Running a full git forge just for CI is excessive when we already use GitHub for hosting and only need a timer.
* **Cron** — Rejected. Works, but systemd timers provide better logging (`journalctl`), failure tracking (`systemctl status`), resource limits (`CPUQuota`), `Persistent=true` for missed runs, and calendar scheduling that doesn't require crontab syntax.
* **Jenkins** — Rejected. Heavyweight Java application. Requires its own JVM, web server, plugin ecosystem. Massive operational overhead for a single pipeline.

## Decision Outcome

Chosen option: "Use a systemd timer and a oneshot service to trigger the build script on a 6-hour cadence", because it has zero cost, zero external dependencies, and provides journald logging, lifecycle tracking, and resource limits out of the box on hardware we already own.
...
* Good, because zero cost — uses existing hardware with no external services.
* Good, because zero external dependencies — no GitHub runner agent, no CI server, no cloud accounts.
* Good, because build logs are available via `journalctl -u ruflo-sync` with full systemd journal features (timestamps, filtering, persistence).
* Good, because `Persistent=true` ensures builds that were missed (e.g., server was off) run immediately on next boot.
* Good, because `CPUQuota=800%` limits the build to 8 cores, leaving the remaining 24 cores available for other work.
* Good, because `Type=oneshot` means systemd tracks the full lifecycle — `systemctl status` shows last run time, exit code, and duration.
* Good, because manual trigger is trivial: `sudo systemctl start ruflo-sync.service`.
* Bad, because no web dashboard — you cannot view build history in a browser. Mitigation: the publish gate (ADR-0010) uses GitHub prereleases as a review UI, and `journalctl` is sufficient for debugging.
* Bad, because it is tied to this specific server — if the server goes down, builds stop. Mitigation: `Persistent=true` catches up on reboot, and the publish gate means users on `@latest` are unaffected by downtime.
* Bad, because it requires root access to install the timer initially (one-time `systemctl enable`).
