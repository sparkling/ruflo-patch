...
| R7 | Bug 7 (write-arm `addEdge` → `addCausalEdge` typo) | **DEFERRED → realised by [[ADR-0276]]** (re-converge ADR edges onto the CausalMemoryGraph controller; resolves the backend-uncertainty as SQLite-only). Original deferral cited backend uncertainty + read-arm asymmetry (see R7 §Trade-off) | `memory-router.ts:1810` (TODO comment landed at 1818-1834) |
