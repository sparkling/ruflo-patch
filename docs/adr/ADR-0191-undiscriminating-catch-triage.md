...
|---|---:|---|---|
| A. Optional dynamic `import()` / `requireCjs()` | 7 | `import('pkg')` swallowed | Per-callsite: is the dep *truly* optional? If yes → `tryOptionalImport` helper that discriminates `ERR_MODULE_NOT_FOUND` / `MODULE_NOT_FOUND` and rethrows the rest. If no (production dep) → remove the catch; missing-on-install IS the bug. |
...
| `autopilot-state.ts:322` `agentic-flow/dist/coordination/autopilot-learning.js` | #5 | `agentic-flow` in optionalDeps; internal module path may vary by version |
...
| All 10 callsites (hooks-tools `1015/1046/1064/1090/2075/2970`; memory-tools `495/581/614/629`) | **#5** (revised — see "Post-implementation revision"). Initial reading classified this as #2 (swallowed method bug), reasoning: `getController` at `memory-router.ts:1564` never throws, the typeof guard handles absence, so the catch must be hiding method-body throws. That reading was incomplete. Release-3's `ctrl-routing` acceptance failure proved the catches implement a documented graceful-degradation contract: on cold start, controller method bodies can throw because internal state isn't populated yet, and `route_task` / `memory_search` are required to fall through to the next strategy (static patterns / unranked results / etc.). The catches are the routing chain, not paranoia. Disposition revised to catch+log. | |
...
| **Catch + `console.error` log** — graceful-degradation contract (#5) enforced by integration tests; signal observable via stderr | B (10) | **10** |
