...
| `autopilot-state.ts:322` `agentic-flow/dist/coordination/autopilot-learning.js` | **#5 partial — but the producer is unbuilt** | `agentic-flow` IS in optionalDeps. However, post-release-3 investigation traced the actual failure mode to a different layer: `AutopilotLearning` is referenced by 7 source files, 2 ADRs (058/072), and 1 integration test — **but the producer file was never committed in any repo** (`git log -- '**/autopilot-learning*'` returns empty in both fork and upstream). The discriminating-helper fix this row applies (Phase B Cluster A) tolerates the absence correctly, but it cannot make the feature work because there is no feature. **ADR-0192** (separate) proposes the Phase 1 implementation (~200 lines per ADR-059 scoping). Until ADR-0192 Phase 1 lands, this catch's runtime behavior is "feature off, gracefully reported as `{ available: false }`." |
