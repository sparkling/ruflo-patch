...
- ❌ **ADR-0315 — DELETED 2026-06-11 (not a real issue).** The ADR-tooling "drift" (`adr/<id>` hierarchical key rejected, `mcp__ruflo__` vs `mcp__claude-flow__`, `path:`/`key:`) was an artifact of testing the skill against an **upstream ruflo project** (older `validateIdentifier` rejects `/`; upstream registers `mcp__claude-flow__`), NOT a fork defect — the fork accepts `adr/<id>` (`validateString`, ADR-0281/0285) and registers `mcp__ruflo__`. Per Henrik 2026-06-11: ignore + delete. ADR file removed; no code change. (The Wave-4 noglobal lib that was briefly numbered 0315 is ADR-0316, shipped — see line above.)
