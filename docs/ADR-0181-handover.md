...
- **Phase 7 — controller-persistence landing** (this loop, 7 commits across forks/agentdb + forks/ruflo): wired the 4 deferred agentdb handlers (`reflexion-store`, `skill-create`, `hierarchical-store`, `sona-trajectory-store`) by collapsing the two-database split-brain. Council found cli writes to `<root>/.swarm/memory.db` (AgentDB-owned, schemas via `loadSchemas()`); archivist reads were querying `<root>/.claude-flow/archivist.db` (separate empty DB). Fix: substrate-registry roster move (3 storeIds RVF→SQLite carve-out), recall axis-flip + SQL port (importance-rank, no MATCH because `hierarchical_memory` schema has no embedding column), `ensureSqliteWired` handle-share (looks up cli's existing `ControllerRegistry.getAgentDB().database` — single handle, forces lazy registry init that creates memory.db). Took 3 release rounds: r1 path-repoint regressed (6 fails — startup-ordering); r2 handle-share fixed 5 (1 fail — read-side gate missed); r3 read-side gate flip closed the last one (0 fails). Net: **6 probes flipped skip_accepted → PASS** (adr0112-27-1/3/4, adr0178-hquery-e2e, p13-agentdb-reflexion, p13-agentdb-skill). Sona deferred per Phase 7 plan — service is in-memory by design, needs separate ADR for persistence + read-handler.
  - **8 heavy-skip drift** (passed → skip_accepted): p4-br-interaction/navigation/snapshot, p7-fo-neural, p8-inv1-memory, t1-2-learning, t3-1-bulk-corpus, t3-4-reasoningbank — these are exactly the documented `_HEAVY_CHECK_IDS` list. Explanation (no mystery): the baseline log (`adr0181-phase8-r1-pre-heavy-skip.log`) was captured at 2026-05-15 18:50, and `d8c26f0` (the commit that introduced `_HEAVY_CHECK_IDS`) landed 7 minutes later at 18:57:52 — the heavy-skip mechanism literally didn't exist when the baseline ran. r3 (post-d8c26f0) correctly skips them.
...
| **Closure-plan amendment — Phase D** | ✅ done via [ADR-0184](adr/ADR-0184-hive-mind-consensus-handler-port.md) close-out (2026-05-18); cli retirement spun to [ADR-0185](adr/ADR-0185-hive-mind-consensus-cli-retirement.md) | All 6 per-strategy bodies live in `forks/agentdb/src/archivist/handlers/hive-mind/consensus/<strategy>.ts`; zero `pending` stubs; audit-entry = mutation count verified by Wave 6a exit-gate test; ~1500-LoC cli retirement scope spun out to ADR-0185 |
...
**Post-handover incremental work (2026-05-17, /loop):** task #99 (DA CF#4 memory_search_index → memory_store collapse) landed in 2 commits — `forks/agentdb 69ddfba` (ReadOnlySubstrateHandle gained `getByKey`+`list`; 3 substrate impls + 22 unit tests) and the follow-on (4 memory handlers flipped from `memory_search_index` to `memory_store`; 30 new handler tests; cli envelope shape preserved). The 4 handlers are **READY** for cli dispatch but not yet called — the cli flip remains task #100. Plan: [docs/council/ADR-0181-task-99-substrate-seam-plan.md](council/ADR-0181-task-99-substrate-seam-plan.md). The "delete memory_search_index storeId" 3rd commit collapsed to a no-op — recon found it was never explicitly registered (classifyStore fallthrough). 6 migration-history comments in the 4 handlers retained as scar tissue. Task #100 inherits the acceptance-coverage extension (originally commit 3) since end-to-end gating requires the cli flip. Two semantic carry-forwards for #100 reviewers: search.ts namespace post-filter uses NS_OVERFETCH=4 (could miss tail at large per-namespace corpora); search-unified.ts collapsed per-namespace iteration to single multi-namespace vectorSearch with UNIFIED_TOPK_MULTIPLIER=8. Both equivalent to cli's existing aggregation rule per the agent's trace; if acceptance flags either, the substrate `vectorSearch` seam needs a namespace-filter extension (new sub-task).

**Release-verified end-state (2026-05-17, patch.166):** `npm run release` against the post-/loop tree returned **668 / 677 PASS / 0 FAIL / 9 skip_accepted**. The -1 check delta vs patch.143 (669/678) is `adr0177-flag-bge-768` — intentionally dropped per task #118 (ADR-0182 L12, completed earlier), not a regression. Strict exit criterion still met (0 fail; 9 skips = exact `_HEAVY_CHECK_IDS` set). Confirms commits `forks/agentdb 69ddfba` + follow-on substrate-seam + handler-flip work end-to-end through the publish pipeline. The handlers' new `memory_store` reads pass acceptance even before the cli flips through `dispatchRead` because nothing in production dispatches to them today (activation gate is task #100).

**Task #100 attempts + reverts (2026-05-17):**

*Attempt 1 (patch.168):* The cli flip (forks/ruflo `c5fd0fe33` — replace 4 memory-read cases' `storage.*` direct calls with `archivist.dispatchRead`) + 4 dispatch acceptance checks (`adr0181-disp-{get,list,search,sunified}`, ruflo-patch `0574f06`) + 3 test-shape updates (ruflo-patch `c8db8ab`). Unit tests passed; release returned **5 acceptance FAILures** in memory semantic round-trip checks: `adr0069-bug3-persist`, `p8-inv12-mem-full`, `e2e-0059-mem-search`, `e2e-0059-p3-unified-both`, `e2e-0059-p3-dedup`. Reverted in 3 commits per `feedback-data-loss-zero-tolerance`. Revert verified at patch.169 = 668/677/0/9 (identical to .166 baseline).

*Root-cause investigation (between attempts):* Identified that `makeCliEmbeddingScorer.embed(text)` was dropping `{intent}` opts when forwarding to `generateEmbedding`. The cli's pre-flip search called `generateEmbedding(text, {intent:'query'})` which applies an asymmetric task prefix via `agentdb.applyTaskPrefix(text, intent)`. Without the prefix, query vectors land in the wrong subspace from stored document vectors. **Intent fix landed in 2 commits:** `forks/agentdb b91b4fd` extends `EmbeddingScorer.embed` to accept `opts?: {intent?: 'query'|'document'}` and has `handlers/memory/search.ts` pass `{intent:'query'}`; `forks/ruflo 0eacaf6ec` forwards intent through `makeCliEmbeddingScorer`. 86/86 unit tests pass with the fix.

*Attempt 2 (patch.170, with intent fix in place):* Re-applied the 3 reverted commits via `git revert <revert-sha>`. Release returned **the same 5 failures** — intent fix was NOT the root cause (or not the only one). Re-reverted in 2 commits (`forks/ruflo dcb2cfc5c` + `ruflo-patch b104efb` — bundles both ruflo-patch reverts in one).
...
**Decision on read-handler invariants (task #104, 2026-05-17):** **defer extending `RegisterReadOpts` with `invariants`.** Rationale:

1. ADR-0180 §Read-path return shape (line 102) already statically types reads as `RankedResults<T>` with `RankedResult` carrying `{item, score, provenance: {storeId, matchType, rawScore, rank, ...}}`. The `tsc --noEmit` boundary already rejects wrong return shapes; a runtime invariant would be redundant for the shape-conformance use case.
2. ADR-0180 §25 ("Reference-impl differential testing") **explicitly defers** the natural home for read-side correctness gates: a reference implementation per high-risk surface that differential-tests the production handler. The candidate set is small (`agentdb_filtered_search`, `SkillLibrary.consolidateEpisodesIntoSkills`, `NightlyLearner.run()`); the broad case is over-engineering by ADR's own analysis. Adding `invariants` to `RegisterReadOpts` would re-open that decision under a different name.
3. Mutation invariants today are 6/100+ wired and still mostly tautological (callerIntent === recordedPayload). Extending the same primitive to reads before the mutation side is fully utilized would broaden surface area without adding catch capacity.

**Trigger to revisit:** any of ADR-0180 §25's three reactivation triggers fires — (a) an invariant-passing handler regression slips to production, (b) Phase 9 load tests reveal divergence at one of the three candidate surfaces, (c) ADR-0179 Phase 3 restoration surfaces a correctness bug in the six lost features. When triggered, the response is the deferred reference-impl program (per §25), not a `RegisterReadOpts.invariants` addition; the reference impl differential-test gates at the §Confirmation boundary rather than per-call.
...
- **8 heavy-skip drift between baseline and Phase 7 r3 — resolved.** Pre-Phase-7 baseline log was captured at 2026-05-15 18:50; `d8c26f0` (`feat(acceptance): default-skip 9 heavy passing tests`) landed 7 minutes later at 18:57:52. The heavy-skip mechanism literally did not exist when the baseline ran — that's why it ran every check including the heavy ones. Phase 7 r3 (post-`d8c26f0`) correctly skips them. Baseline (658) is therefore the *honest* number for "with heavies", Phase 7 (656) is the *honest* number for "without heavies"; current 669 is also honest. Log retained for archaeology as `adr0181-phase8-r1-pre-heavy-skip.log`.
...
- **Slow-check audit (task #111, 2026-05-17)** — 32 acceptance checks pass >5s in baseline; classified into 15 HEAVY-eligible + 12 CRITICAL (keep) + 4 INSTRUMENT (flaky, fix first). **Decision: don't bulk-add to `_HEAVY_CHECK_IDS`** — net savings are 25-40s/release (modest due to parallelism), not worth reducing default coverage. The deeper finding: **4 patterns** suggest structural fixes outpace per-check skipping —
  1. **P8 invariants dominate** (5 of top 12 slowest). Twin to `p8-inv1-memory` already heavy. All share setup overhead; a shared-init fixture would beat per-check skipping.
  2. **ADR-0177 embedding-model roundtrips** (3 sibling checks 7.4–8.2s, regression-sensitive). Candidate for shared model-warm fixture.
...
#    - (Heavy-skip baseline "mystery" — RESOLVED in Section K: baseline log
#      was captured 7 minutes before the heavy-skip feature commit landed)
