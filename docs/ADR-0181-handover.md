...
**Task #100 attempt + revert (2026-05-17, patch.168 → revert verified at patch.169):** The cli flip (forks/ruflo `c5fd0fe33` — replace 4 memory-read cases' `storage.*` direct calls with `archivist.dispatchRead`) was attempted in a follow-on /loop iteration. Unit tests passed after test-shape updates pinning the new dispatched pattern (commit `c8db8ab` on ruflo-patch). 4 dispatch-coverage acceptance checks were added (`adr0181-disp-{get,list,search,sunified}`, commit `0574f06`). End-to-end release at patch.168 returned **5 acceptance FAILures**: `adr0069-bug3-persist`, `p8-inv12-mem-full`, `e2e-0059-mem-search`, `e2e-0059-p3-unified-both`, `e2e-0059-p3-dedup` — all memory-read semantic round-trip checks (store value → semantic search → must find stored key). Per `feedback-data-loss-zero-tolerance`, the cli flip was **reverted** in 3 commits: `forks/ruflo 36152e3ba` + `ruflo-patch ee0ce85 + 2b46bde`. The 4 dispatch acceptance checks + test-shape updates were reverted alongside the cli flip to keep coupling together. #99 commits 1+2 (substrate-seam + handler activation) remain landed as additive safe work. **Revert verified at patch.169: 668/677/0/9** — identical to patch.166 baseline; clean restoration. **Root-cause hypothesis for #100 re-attempt:** the dispatched handler's `ctx.capabilities.requireEmbeddingScorer().embed(text)` likely produces semantically different vectors than the cli's pre-flip inline `generateEmbedding(op.query, {intent:'query'})` call, OR the `NS_OVERFETCH=4` namespace post-filter in `search.ts` loses results vs the pre-flip storage.search's pushed-down namespace filter, OR `MemoryRvfAdapter.queryAsync` semantic differs from `RvfBackend.search`. Pre-flight for the next attempt: A/B embedding-output comparison on identical query strings; A/B vectorSearch result comparison; namespace-filter semantic verification with a >NS_OVERFETCH×topK out-of-namespace corpus.
