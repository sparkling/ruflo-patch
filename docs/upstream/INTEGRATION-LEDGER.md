...
| **Batch S remainder (~65 commits)** | 2026-05-18..05-22 | ADR-123 followups (sublinear-time-solver ADR docs, 2 commits remaining) + ADR-125 memory hybrid (9) + ADR-126 neural-trader (10) + ADR-127 GitHub surface (7) + supply-chain #2046 followups (6) + kg-extract #2049 (1) + guidance perf #2103 (3) + ADR-122 lockfile/ci refreshes (4) + docs (5) + other (14) | deferred | — | 0228 | Each ADR family hits its own conflict patterns (lockfile/ci.yml/package.json fork-pin overrides) + plugin code adding new directories. Cherry-pick conflicts on these are dense enough that bulk processing risks silent-edit bugs like the agentic-synth marker incident (commit `30eeb68ba`). Deferred to a follow-up session for per-family controlled triage. |
