...
| **D** — `hive-mind/consensus.ts` stub port | **Completed via [ADR-0184](../adr/ADR-0184-hive-mind-consensus-handler-port.md) close-out (2026-05-18); cli retirement spun to [ADR-0185](../adr/ADR-0185-hive-mind-consensus-cli-retirement.md).** All 6 per-strategy bodies live in `forks/agentdb/src/archivist/handlers/hive-mind/consensus/<strategy>.ts` (bft / raft / quorum / weighted / gossip / crdt); zero `pending` stubs remain in the agentdb handler tree per the Wave 6a exit-gate test; audit-entry count = mutation count verified. ADR-0184 ran 6 waves with 0 acceptance regressions (672/681/0/9 sustained throughout). The cli `hive-mind_consensus` handler is now 100% redundant with the agentdb handler — both work correctly in dual-write. The cli retirement itself (a comprehensive `buildConsensusResponse` reconstructing ~20 telemetry fields per action × 4 actions × 6 strategies, plus helper-set cleanup) was scope-measured at ~1500 LoC and spun out to ADR-0185 per the ADR-0183 spin-out precedent. | All 6 strategy handlers under `forks/agentdb/src/archivist/handlers/hive-mind/consensus/` (committed Wave 1-5; Wave 6a exit-gate test in `test/archivist/handlers/exit-gate.test.ts`). Close-out report: [docs/council/ADR-0184-close-out-report.md](ADR-0184-close-out-report.md). |
