...
| **D** — `hive-mind/consensus.ts` stub port | **Deferred to own ADR.** The cli implementation of `hive-mind_consensus` (`forks/ruflo/v3/@claude-flow/cli/src/mcp-tools/hive-mind-tools.ts`) spans 926 LoC across 7 strategies (bft / raft / quorum / weighted / byzantine / gossip / crdt) × 4 actions (propose / vote / status / list) with strategy-specific merge / tally / quorum logic (CRDT merges, Raft term updates, Byzantine voting, ADR-0119 weighted queen 3x, ADR-0120 gossip rounds with `roundTimeoutMs`, ADR-0121 CvRDT `mergeCRDTState`, ADR-0131 auto-status-transition). The cli surface works today — its own comment header says "cli logic below stays load-bearing". The archivist handler stub at `forks/agentdb/src/archivist/handlers/hive-mind/consensus.ts` (148 LoC: types + 1 stub throw) is structurally complete but unused. **Reason for defer:** single-threaded queen-as-implementer port (the in-process Agent-tool constraint, see §"Execution model" below) on a 926-LoC strategy fan-out without parallel review is high-risk for one-shot success — likely 2-3 release cycles of trace-and-revert per strategy. Deferral pattern mirrors ADR-0183 (peel out a single concern when the closure plan would otherwise bloat). | Existing stub at `forks/agentdb/src/archivist/handlers/hive-mind/consensus.ts`. Recommend new ADR-0184: "Hive-Mind Consensus Archivist Port" — split per-strategy modules under `handlers/hive-mind/consensus/<strategy>.ts` + port from cli. |
...
1. **ADR-0184 (proposed): Hive-Mind Consensus Archivist Port.** Port the 926-LoC cli strategy fan-out (7 strategies × 4 actions) into per-strategy modules under `forks/agentdb/src/archivist/handlers/hive-mind/consensus/<strategy>.ts`. Exit: zero `pending` stubs in `handlers/**`; cli flips through dispatch; all strategy-specific invariants land.
