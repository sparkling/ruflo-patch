...
check_e2e_init_no_dead_files() {
  ...
  # ADR-0177: pglite substrate retired (RVF-first). PG_VERSION check removed.
  ...
  if [[ -z "$errors" ]]; then
    _CHECK_PASSED="true"
    _CHECK_OUTPUT="E2E-6: correct file layout — RVF present, no dead .claude/memory.db or .swarm/memory.graph"
  else
