...
# ════════════════════════════════════════════════════════════════════
# Scenario 1 (§4a, SUPERSEDED by ADR-0155):
#   ADR-0104 §4a originally specified that .mcp.json should pin to a
#   directly-resolved global ruflo binary path when one was on PATH,
#   optimising the npx cold-start. ADR-0155 (2026-05-07) reverts that
#   optimisation: the freshness loss from a stale globally-installed
#   wrapper is the worse default. Per `feedback-always-npx-for-ruflo`,
#   `.mcp.json` MUST unconditionally use:
#       "command": "npx",
#       "args":    ["-y", "@sparkleideas/ruflo@latest", "mcp", "start"]
#   regardless of whether ruflo is on PATH.
#
#   The check name is preserved (`check_adr0104_mcp_direct_path`) so the
#   runner wiring stays stable, but the contract it pins is the post-
#   ADR-0155 contract.
# ════════════════════════════════════════════════════════════════════
check_adr0104_mcp_direct_path() {
  ...
  _CHECK_OUTPUT=""
  if [[ -z "$CLI_BIN" ]]; then
    _CHECK_OUTPUT="ADR-0155: CLI_BIN not set"
    return
  fi
  ...
  if [[ ! -f "$iso/.mcp.json" ]]; then
    _CHECK_OUTPUT="ADR-0155: .mcp.json not generated by init"
    rm -rf "$iso" 2>/dev/null; return
  fi

  local cf_cmd cf_args
  cf_cmd=$(python3 -c "import json; d=json.load(open('$iso/.mcp.json')); print((d.get('mcpServers',{}).get('ruflo',{}) or {}).get('command',''))" 2>/dev/null)
  cf_args=$(python3 -c "import json; d=json.load(open('$iso/.mcp.json')); print(' '.join((d.get('mcpServers',{}).get('ruflo',{}) or {}).get('args',[])))" 2>/dev/null)

  if [[ "$cf_cmd" != "npx" ]]; then
    _CHECK_OUTPUT="ADR-0155: .mcp.json command must be 'npx', got '$cf_cmd' (ADR-0104 §4a direct-path is reverted)"
    rm -rf "$iso" 2>/dev/null; return
  fi

  if ! echo " $cf_args " | grep -qF " @sparkleideas/ruflo@latest "; then
    _CHECK_OUTPUT="ADR-0155: .mcp.json args must include @sparkleideas/ruflo@latest, got '$cf_args'"
    rm -rf "$iso" 2>/dev/null; return
  fi

  if ! echo " $cf_args " | grep -qF " -y "; then
    _CHECK_OUTPUT="ADR-0155: .mcp.json args must include -y (npx auto-confirm), got '$cf_args'"
    rm -rf "$iso" 2>/dev/null; return
  fi

  _CHECK_PASSED="true"
  _CHECK_OUTPUT="ADR-0155: .mcp.json ruflo.command='npx' args='$cf_args' (npx-@latest unconditional)"
