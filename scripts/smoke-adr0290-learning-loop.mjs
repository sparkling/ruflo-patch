 ...
 *   A3 — METADATA-ONLY (PII gate): the episode row contains NO token of the
 *        description or prompt (canary strings absent), free-text columns
 *        (input/output/code/critique) are NULL, the task label is the derived
 *        type slug, and NO episode_embeddings row was written (skipEmbedding —
 *        ADR-0287 F10 constraint 4: no per-task embed in the capture path).
...
async function main() {
  ...
  // the prompt (never even forwarded).
  const descCanary = `DESC-CANARY-${runTag}`;
  const promptCanary = `PROMPT-CANARY-${runTag}`;
