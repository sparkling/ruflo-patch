...
if [[ -z "$CLI_BIN" ]]; then
  echo "[fast] FATAL: CLI not found at ${ACCEPT_TEMP}/node_modules/.bin/{ruflo,claude-flow,cli}"; exit 1
fi
...
if [[ "$_FAST_RUN_GROUPS" == *"adr0266"* || "$_FAST_RUN_GROUPS" == "all" ]]; then
  if [[ -f "$PROJECT_DIR/lib/acceptance-adr0266-checks.sh" ]]; then
    source "$PROJECT_DIR/lib/acceptance-adr0266-checks.sh"
    echo "── ADR-0266 (ADR-129 Phases 1-3 implementation amendment) ──"
    if ! _adr0266_setup_shared_temp; then
      echo "[fast] ADR-0266 shared-temp setup FAILED — aborting" >&2
      exit 1
    fi
    echo "[fast] adr0266 shared-temp: ${ADR0266_SMOKE_SHARED_TEMP}"
    _fast_run "adr0266-group1-introspection" check_adr0266_group1_introspection
    _fast_run "adr0266-group2-reset"         check_adr0266_group2_reset
    _fast_run "adr0266-group3-gallery"       check_adr0266_group3_gallery
...
if [[ "$_FAST_RUN_GROUPS" == *"adr0279"* || "$_FAST_RUN_GROUPS" == "all" ]]; then
  if [[ -f "$PROJECT_DIR/lib/acceptance-adr0279-checks.sh" ]]; then
    source "$PROJECT_DIR/lib/acceptance-adr0279-checks.sh"
    echo "── ADR-0279 (episodes carry an action dimension) ──"
    export ADR0255_SMOKE_SHARED_TEMP="$ACCEPT_TEMP"
    echo "[fast] adr0279 reusing ACCEPT_TEMP: ${ACCEPT_TEMP}"
    _fast_run "adr0279-episodes-action-dimension" check_adr0279_episodes_action_dimension
    unset ADR0255_SMOKE_SHARED_TEMP
  fi
fi
...
if [[ "$_FAST_RUN_GROUPS" == *"adr0281"* || "$_FAST_RUN_GROUPS" == "all" ]]; then
  if [[ -f "$PROJECT_DIR/lib/acceptance-adr0281-checks.sh" ]]; then
    source "$PROJECT_DIR/lib/acceptance-adr0281-checks.sh"
    echo "── ADR-0281 (hierarchical keyed upsert + delete-by-key) ──"
    export ADR0255_SMOKE_SHARED_TEMP="$ACCEPT_TEMP"
    echo "[fast] adr0281 reusing ACCEPT_TEMP: ${ACCEPT_TEMP}"
    _fast_run "adr0281-hierarchical-upsert-delete" check_adr0281_hierarchical_upsert_delete
    unset ADR0255_SMOKE_SHARED_TEMP
  fi
fi
