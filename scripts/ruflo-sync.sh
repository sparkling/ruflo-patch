...
_fetch_upstream_parallel() {
  ...
  for i in "${!FORK_NAMES[@]}"; do
    local dir="${FORK_DIRS[$i]}"
    [[ -d "${dir}/.git" ]] || continue
    local _ub; _ub=$(_upstream_branch "${FORK_NAMES[$i]}")
    git -C "${dir}" fetch upstream "$_ub" --quiet 2>/dev/null &
    upstream_fetch_pids+=($!)
  done
...
_create_sync_branches() {
  ...
  for i in "${forks_to_sync[@]}"; do
    ...
    log "Creating sync branch ${branch_name} in ${name} (tracking upstream/${_ub})"
    ...
    local _merge_start
    _merge_start=$(_ns)
    if ! git -C "${dir}" merge --no-edit "upstream/${_ub}" 2>/dev/null; then
    ...
    log "Merged upstream/${_ub} into ${branch_name} for ${name}"

    local upstream_sha
    upstream_sha=$(git -C "${dir}" rev-parse "upstream/${_ub}" 2>/dev/null) || true
...
sync_upstream() {
  ...
  for i in "${!FORK_NAMES[@]}"; do
    ...
    upstream_sha=$(git -C "${dir}" rev-parse "upstream/${_ub}" 2>/dev/null) || continue
