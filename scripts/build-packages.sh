...
run_build() {
  ...
  # Bug-6 (2026-05-06) FIX: replaced unconditional wipe with rsync-side
  # exclusion in scripts/copy-source.sh (`--exclude='*.tsbuildinfo'
  # --exclude='tsconfig.tsbuildinfo'`). Fork-side stale tsbuildinfo files
  # (left by developer-machine `tsc --build` runs) no longer reach
  # /tmp/ruflo-build, so the per-package smart invalidation at line ~138
  # below is sufficient to catch genuine source changes. The destination
  # tsbuildinfo (written by THIS pipeline's tsc invocation) persists via
  # the `--filter='P .tsbuildinfo'` PROTECT directive in copy-source.sh,
  # enabling fast incremental rebuilds across releases.
  #
  # Win: ~10-30s wall time savings per release (only changed packages
  # recompile) + ~6k fewer file-emit FSEvents per release. Symptom-vector
  # closed at the rsync boundary instead of the build boundary.
