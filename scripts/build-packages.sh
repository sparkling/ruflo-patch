...
run_build() {
  ...
  # Bug-6 (2026-05-06): unconditionally wipe ALL tsc incremental cache files
  # before tsc runs. The per-package mtime-invalidation logic at line ~121
  # only checks `${pkg_dir}/.tsbuildinfo` literally, but tsc with
  # `tsc --build` (used elsewhere in the toolchain) writes to
  # `${pkg_dir}/tsconfig.tsbuildinfo` (different filename) — that file gets
  # rsync-copied from the fork tree (where it was generated by some prior
  # local build) and read by subsequent tsc invocations, producing stale
  # output even when the source has changed. Symptom: my Bug-1 HNSW
  # orphan-self-heal fix in rvf-backend.ts shipped to forks/ruflo at
  # commit 71b2ad33e, but the published `@sparkleideas/memory` dist did
  # NOT contain the fix because tsc fast-pathed off the stale cache.
  # Cost of unconditional wipe: ~10-30s per release (full recompile);
  # benefit: every published artifact reflects the current source.
  local _wiped
  _wiped=$(find "${TEMP_DIR}" \( -name "*.tsbuildinfo" -o -name "tsconfig.tsbuildinfo" \) -not -path "*/node_modules/*" -delete -print 2>/dev/null | wc -l | tr -d ' ')
  if [[ "${_wiped:-0}" -gt 0 ]]; then
    log "  wiped ${_wiped} stale tsbuildinfo cache files (forces full tsc recompile)"
  fi
