...
// Seed 4 golden rows directly via sqlite. Memory_entries IDs are integer
// FKs; we insert dummy memory_entries first then the edges referencing them.
function seedGoldenRows(dbPath) {
  log(`\n[seed] inserting 4 golden memory_entries + edges`);
  // Use 4 distinct memory_entries; bind explicit ids. Schema is shared
  // with upstream — id INTEGER PRIMARY KEY, namespace TEXT, key TEXT,
  // value TEXT, …
  const seedSql = `
    INSERT OR REPLACE INTO memory_entries (id, namespace, key, value)
      VALUES (1001, 'smoke', 'auth-module', 'authentication module');
    INSERT OR REPLACE INTO memory_entries (id, namespace, key, value)
      VALUES (1002, 'smoke', 'jwt-lib', 'jwt signing library');
    INSERT OR REPLACE INTO memory_entries (id, namespace, key, value)
      VALUES (1003, 'smoke', 'user-db', 'user database');
    INSERT OR REPLACE INTO memory_entries (id, namespace, key, value)
      VALUES (1004, 'smoke', 'rate-limit', 'rate limiter');
    INSERT INTO graph_edges (source_id, target_id, relation, weight, confidence, decay_rate, last_reinforced)
      VALUES (1001, 1002, 'depends-on', 0.9, 0.95, 0.01, strftime('%s','now'));
    INSERT INTO graph_edges (source_id, target_id, relation, weight, confidence, decay_rate, last_reinforced)
      VALUES (1001, 1003, 'reads-from', 0.8, 0.90, 0.01, strftime('%s','now'));
    INSERT INTO graph_edges (source_id, target_id, relation, weight, confidence, decay_rate, last_reinforced)
      VALUES (1002, 1004, 'protected-by', 0.7, 0.85, 0.01, strftime('%s','now'));
    INSERT INTO graph_edges (source_id, target_id, relation, weight, confidence, decay_rate, last_reinforced)
      VALUES (1001, 1004, 'uses', 0.6, 0.80, 0.01, strftime('%s','now'));
  `;
...
// Invoke MCP tool via cli (call into stdio JSON-RPC); fall back to direct
// inspection if the MCP path is not yet wired. The fork ships a stdio MCP
// server at `cli mcp start`; we issue tools/call JSON-RPC requests.
function mcpToolCall(cli, tempDir, toolName, args) {
  ...
  // Fallback: many fork builds expose `cli mcp call <tool> <args>` instead.
  if (r.status !== 0) {
...
function parseJsonish(out) {
  // CLI MCP wrappers may print JSON-RPC envelope or bare JSON; strip
  // anything before the first { to be robust to leading log noise.
  const i = out.indexOf('{');
  if (i < 0) return null;
...
function main() {
  ...
  try {
    ...
    const khop = mcpToolCall(cli, tempDir, 'agentdb_graph-query', {
      nodeId: '1001', mode: 'k-hop', depth: 2,
    });
    ...
    if (khopJson) {
      const results = khopJson.results || khopJson.result?.results || [];
      assert(Array.isArray(results), '1c: results is an array',
        `got: ${typeof results}`);
      assert(results.length > 0, '1d: k-hop returns non-empty results',
    ...
    const pr = mcpToolCall(cli, tempDir, 'agentdb_graph-query', {
      nodeId: '1001', mode: 'pagerank', topK: 5,
    });
    ...
    if (prJson) {
      const results = prJson.results || prJson.result?.results || [];
      assert(Array.isArray(results), '2c: results is array');
      assert(results.length > 0, '2d: pagerank returns non-empty results',
        `length=${results.length}`);
      if (results.length > 0 && results[0]) {
        assert(typeof results[0].score === 'number' ||
          typeof results[0].rank === 'number' ||
          typeof results[0].id !== 'undefined',
          '2e: first result has score/rank/id shape',
    ...
    const sem = mcpToolCall(cli, tempDir, 'agentdb_graph-query', {
      nodeId: '1001', mode: 'semantic', topK: 5,
    });
    ...
    if (semJson) {
      const results = semJson.results || semJson.result?.results || [];
      assert(Array.isArray(results),
        '3c: semantic results is array');
      // Semantic mode may return empty when no embedding_ref is populated
      // on the golden seeds (Agent B may seed them later). Accept either,
      // but require the shape to be well-formed.
