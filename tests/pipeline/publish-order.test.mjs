...
const KNOWN_DEPS = {
  ...
  '@sparkleideas/ruvector-attention-wasm': [],
...
describe('Topological publish order (ADR-0014)', () => {
  ...
  describe('Package completeness', () => {
    it('all expected packages are present across all levels (27+5+5+22+2)', () => {
      const allPackages = LEVELS.flat();
      // ADR-0014 + ADR-0071 + F3 + W3 + ADR-0113 Fix 5 + ADR-0239 cleanup + ADR-0242 errors + ADR-0265 quic-native + Phase 2a sub-package.
      // L1=27 (was 26; +agentic-flow-quic-native-darwin-arm64 ADR-0265 Phase 2a),
      ...
      // L5=2. Total: 61 (was 60; +1 darwin-arm64 sub-package).
      assert.equal(LEVELS[0].length, 27, 'Level 1 should have 27 packages (ADR-0265 Phase 2a added agentic-flow-quic-native-darwin-arm64)');
      ...
      assert.equal(
        allPackages.length,
        61,
        `Expected 61 packages total (62 pre-ADR-0239, -4 deleted, +errors ADR-0242, +quic-native ADR-0265, +darwin-arm64 sub-package), got ${allPackages.length}`
      );
    });
