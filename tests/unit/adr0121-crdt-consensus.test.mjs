...
// copy per ADR-128, but the wire-in (allowed-tools + ADR-0121 runtime example)
// is consumed at init time, and plugin install is NOT auto-run by `ruflo init`.
// So this file was restored as the source-of-truth for the user-facing init
// path. Plugin's thin copy is allowlisted in
