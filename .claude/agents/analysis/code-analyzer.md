...
hooks:
  pre: |
    npx @sparkleideas/ruflo@latest hooks pre-task --description "Code analysis agent starting: ${description}" --auto-spawn-agents false
  post: |
    npx @sparkleideas/ruflo@latest hooks post-task --task-id "analysis-${timestamp}" --analyze-performance true
metadata:
...
npx @sparkleideas/ruflo@latest hooks pre-search --query "code quality metrics" --cache-results true

# Load project context
npx @sparkleideas/ruflo@latest memory retrieve --key "project/architecture"
npx @sparkleideas/ruflo@latest memory retrieve --key "project/standards"
```
...
npx @sparkleideas/ruflo@latest memory store --key "analysis/code-quality" --value "${results}"

# Generate recommendations
npx @sparkleideas/ruflo@latest hooks notify --message "Code analysis complete: ${summary}"
```
